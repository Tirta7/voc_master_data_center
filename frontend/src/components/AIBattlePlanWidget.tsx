        ...
        <div className="bg-[#0F172A] border border-indigo-500/20 rounded-[2rem] p-6 shadow-2xl overflow-hidden relative mb-8 backdrop-blur-xl">
            ...
            <div className={`flex items-center justify-between ${isMinimized ? '' : 'mb-6'}`}>
                ...
                <div className="flex items-center gap-3 z-10">
                    ...
                    <div className="px-3 sm:px-4 py-2 bg-indigo-500/10 rounded-xl border border-indigo-500/20">
                        <span className="text-[10px] sm:text-[11px] font-black text-indigo-400 uppercase tracking-widest">
                            Goal: Rp {(Number(battlePlan.targetRevenue || 0)).toLocaleString('id-ID')}
                        </span>
                    </div>
                    <button 
                        onClick={() => setIsMinimized(!isMinimized)}
                        className="p-2 bg-white/5 rounded-xl border border-white/10 hover:bg-white/10 transition-all active:scale-95"
                    >
                        <ChevronDown className={`w-4 h-4 text-slate-400 transition-transform duration-300 ${isMinimized ? 'rotate-180' : ''}`} />
                    </button>
                </div>
            </div>
