...
const ScratchBox = memo(({ index, value, isOpened, onClick }: any) => {
 ...
 <div
 onClick={handleClick}
 className={`relative w-full aspect-square rounded-xl border-2 transition-all duration-300 ${localOpen ? "bg-slate-800 border-indigo-500/50" : "bg-indigo-950 border-white/10 hover:border-white/20 cursor-pointer"
 }`}
 >
 {localOpen && (
...
export default function SmartDisplay() {
 return (
 <Suspense fallback={<div className="min-h-screen bg-[#000000] flex flex-col items-center justify-center p-8 text-center text-white"><Loader2 className="w-10 h-10 animate-spin text-[#0A84FF] mb-4" /><p className="font-black uppercase tracking-widest text-xs opacity-50">Initializing Terminal Display...</p></div>}>
 <SmartDisplayContent />
 </Suspense>
 );
}
...
function SmartDisplayContent() {
 ...
 <div className="absolute inset-0 overflow-hidden pointer-events-none z-0 transform-gpu bg-[#000000]">
 <BackgroundParticles />
 <motion.div
 animate={{ 
 x: [0, 50, -50, 0], 
 ...
 className="absolute top-[-20%] right-[-10%] w-[80%] h-[80%] bg-[#0A84FF]/10 blur-[150px] rounded-full will-change-transform"
 />
 <motion.div
 animate={{ 
 x: [0, -40, 40, 0], 
 ...
 if (data) {
 // Only trigger popup if they haven't submitted yet and it's not currently showing
 if (!showRatingPopupRef.current && !ratingSubmittedRef.current) {
 ...
 axios.post(`/public-transactions/${txIdRef.current}/rating`, {
 ...
 setRatingSubmitted(false); // Reset so it's fresh for the next transaction
 }
 };
 ...
 <h3 className={`text-5xl font-black uppercase tracking-tight ${scratchResult?.is_winner ? 'text-amber-500' : 'text-slate-500'}`}>
 ...
 <h2 className="text-3xl font-black uppercase tracking-tight mb-2">COMPLETE</h2>
 ...
 <p className="text-[9px] font-black uppercase tracking-[0.4em] text-[#0A84FF]">Multi-Payment Area</p>
 </div>

 <div className="flex-1 space-y-3 overflow-y-auto pr-2 noscrollbar relative z-10">
 {(splitState.payers || []).map((p: any, i: number) => (
 <div key={i} className="space-y-2">
 <div className={`p-4 rounded-xl flex justify-between items-center transition-all ${p.isActive ? 'bg-[#0A84FF] shadow-xl shadow-indigo-900/40 translate-x-1' : 'bg-white/5 border border-white/10'}`}>
 ...
 {p.billiardPortion > 0 && (
 <div className="flex justify-between items-center text-[9px] text-indigo-300 ">
 ...
 <div className="text-right">
 <p className="text-[7px] font-black text-[#0A84FF] uppercase tracking-widest mb-1">Active Now</p>
 <p className="text-[10px] font-black text-white uppercase ">{splitState.activePayer || 'Waiting...'}</p>
 </div>
 </div>
 </div>
 );
 ...
 {member ? (
 <div className="w-full flex items-center gap-4 p-1 text-[#0A84FF]">
 <motion.div
 ...
 <p className="text-sm sm:text-xl font-black uppercase tracking-tight text-white truncate max-w-[120px] sm:max-w-none">{member.name}</p>
 </div>
 ...
 className="px-4 sm:px-6 bg-[#0A84FF] text-white rounded-xl font-black text-[8px] sm:text-[10px] uppercase tracking-widest disabled:opacity-20 transition-all hover:bg-indigo-500 active:scale-95 shadow-lg shadow-indigo-900/20"
 >
 ...
 return (
 <div className="fixed inset-0 z-10 py-6 sm:py-10 flex flex-col overflow-hidden bg-[#000000] animate-in fade-in zoom-in-95 duration-700">
 {/* Header - Fixed with Intelligence */}
 <div className="flex justify-between items-center px-8 sm:px-14 mb-8 sm:mb-12 shrink-0 py-8 bg-[#000000]/40 border-b border-white/10 shadow-2xl z-20">
 <div className="flex flex-col text-left space-y-3">
 <div className="flex items-center gap-3">
 <div className="w-2 h-2 bg-[#30D158] rounded-full animate-pulse shadow-2xl"></div>
 <p className="text-[10px] sm:text-xs font-black text-[#0A84FF] uppercase tracking-[0.5em] ">System Terminal Monitor</p>
 </div>
 <h2 className="text-3xl sm:text-6xl font-black text-white uppercase tracking-tight leading-none">Tables Floor</h2>
 <div className="flex items-center gap-6 sm:gap-8">
 <div className="flex flex-col">
 <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest">In Use</span>
 <span className="text-xl font-black text-[#0A84FF]">{busyTables - billingTables}</span>
 </div>
 ...
 <span className="text-xl font-black text-[#30D158]">{availableTables}</span>
 </div>
 {maintenanceTables > 0 && (
 <>
 <div className="w-[1px] h-6 bg-white/5"></div>
 ...
 <span className="text-xs font-black uppercase tracking-[0.3em] ">Close</span>
 </button>
 <div className="hidden sm:flex w-20 h-20 bg-white/5 rounded-[2rem] border border-white/10 shadow-2xl items-center justify-center overflow-hidden p-4 group hover:scale-105 transition-transform">
 {settings?.logoPath ? (
 <img src={getFullImageUrl(settings.logoPath)} alt="Logo" className="w-full h-full object-contain" />
 ) : (
 <Zap className="w-10 h-10 text-[#0A84FF]" />
 )}
 </div>
 </div>
 </div>
 ...
 let cardStyle = 'bg-white/[0.02] border-white/10';
 let glowStyle = 'bg-[#30D158]/10';
 ...
 >
 <div className={`relative h-full w-full rounded-[2.9rem] p-6 flex flex-col items-center justify-between gap-6 overflow-hidden border bg-[#0F172A]/80 ${!isAvailable ? '' : 'bg-transparent border-white/10'}`}>
 ...
 <h3 className={`text-4xl sm:text-6xl font-black tracking-tight leading-none transition-all duration-500 relative z-10 ${!isAvailable ? 'text-white' : 'text-slate-800'}`}>
 {t.tableName.replace(/[^0-9]/g, '')}
 </h3>
 <p className={`text-[8px] font-black uppercase tracking-[0.5em] mt-3 whitespace-nowrap transition-colors ${!isAvailable ? 'text-[#0A84FF]' : 'text-slate-600/40'}`}>
 Station Monitor
 </p>
 </div>

 <div className={`w-full pt-6 border-t relative z-10 ${!isAvailable ? 'border-white/10' : 'border-white/10'}`}>
 {(isOccupied || isBilling) ? (
 <div className="space-y-3 text-center">
 <p className={`text-[10px] font-black uppercase truncate px-2 leading-none ${isBilling ? 'text-amber-400' : 'text-[#0A84FF]'}`}>
 {customerName || 'GUEST'}
 </p>
 <div className="flex items-center justify-center gap-2 group/time">
 <Timer className={`w-3.5 h-3.5 ${isBilling ? 'text-amber-400' : 'text-[#0A84FF]'} group-hover/time:rotate-12 transition-transform`} />
 <span className="text-lg sm:text-xl font-black text-white tracking-tight tabular-nums">{getDuration(startTime)}</span>
 </div>
 </div>
 ) : isMaintenance ? (
 <div className="text-center py-2">
 <p className="text-[10px] font-black text-rose-500/60 uppercase tracking-widest ">Out of Service</p>
 </div>
 ) : (
 <div className="flex flex-col items-center gap-1">
 <p className="text-[8px] font-black text-[#30D158]/40 uppercase tracking-widest">Ready</p>
 <div className="flex gap-1.5 focus-circle">
 {[1, 2, 3].map(i => <div key={i} className="w-1 h-1 rounded-full bg-[#30D158]/20" />)}
 </div>
 </div>
 )}
 </div>
 ...
 <div className="fixed bottom-24 left-1/2 -translate-x-1/2 z-30 px-10 py-4 bg-white/[0.02] border border-white/10 rounded-[2.5rem] flex items-center gap-8 shadow-2xl">
 <div className="flex items-center gap-3">
 <div className="w-2 h-2 rounded-full bg-[#30D158] shadow-2xl"></div>
 <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest leading-none">Available</span>
 </div>
 <div className="flex items-center gap-3">
 <div className="w-2 h-2 rounded-full bg-indigo-500 shadow-2xl animate-pulse"></div>
 <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest leading-none">In Use</span>
 </div>
 <div className="flex items-center gap-3">
 <div className="w-2 h-2 rounded-full bg-amber-500 shadow-2xl animate-pulse"></div>
 ...
 <div className="flex gap-6 p-3 bg-white/[0.02] rounded-[3rem] border border-white/10 shadow-2xl max-w-lg mx-auto">
 <button
 onClick={() => setEmptyView('WAITING_FORM')}
 className="flex-1 py-4 sm:py-5 bg-[#0A84FF] hover:bg-indigo-500 text-white rounded-2xl sm:rounded-3xl font-black uppercase tracking-widest text-[10px] shadow-xl transition-all flex items-center justify-center gap-3 active:scale-95"
 >
 ...
 className="flex-1 py-4 sm:py-5 bg-white/5 hover:bg-white/10 border border-white/10 text-white rounded-2xl sm:rounded-3xl font-black uppercase tracking-widest text-[10px] transition-all flex items-center justify-center gap-3 active:scale-95"
 >
 ...
 <div className="flex justify-between items-center mb-8 sm:mb-12 border-b border-white/10 pb-8">
 <div className="space-y-3">
 <div className="flex items-center gap-3">
 <div className="w-2 h-2 bg-indigo-500 rounded-full animate-pulse shadow-2xl"></div>
 <p className="text-[10px] sm:text-xs font-black text-[#0A84FF] uppercase tracking-[0.5em] ">Live System Monitor</p>
 </div>
 <h2 className="text-4xl sm:text-7xl font-black text-white uppercase tracking-tight leading-none">Waiting List</h2>
 ...
 <Zap className="w-10 h-10 text-[#0A84FF] group-hover:animate-bounce" />
 )}
 </div>
 </div>
 </div>

 <div className="glass-card rounded-[4rem] p-1 shadow-2xl bg-white/[0.01] border-white/10 w-full flex flex-col max-h-[60vh] overflow-hidden relative">
 ...
 const statusColor = waitMinutes > 30 ? 'bg-rose-500' : waitMinutes > 15 ? 'bg-amber-500' : 'bg-[#30D158]';
 ...
 key={entry.id}
 className="relative group bg-white/[0.02] hover:bg-white/[0.04] border border-white/10 rounded-[2.5rem] p-5 sm:p-7 flex flex-col sm:flex-row justify-between items-center gap-6 transition-all duration-500 hover:translate-y-[-2px] shadow-lg"
 >
 ...
 <div className="flex items-center gap-3">
 <h3 className="text-xl sm:text-3xl font-black text-white tracking-tight uppercase group-hover:text-[#0A84FF] transition-colors">
 {entry.customerName}
 </h3>
 ...
 <div className="flex flex-col items-center sm:items-center gap-3 w-full sm:w-auto pt-4 sm:pt-0 border-t sm:border-t-0 border-white/10">
 <div className="flex items-center gap-3">
 <div className={`w-2 h-2 rounded-full ${statusColor} shadow-2xl`}></div>
 ...
 <p className="text-[10px] font-bold text-[#0A84FF] uppercase truncate max-w-[120px]">
 ...
 <h3 className="text-4xl font-black text-white uppercase tracking-widest leading-none">ALL CLEAR</h3>
 <p className="text-slate-500 max-w-xs mx-auto text-[10px] font-black uppercase tracking-[0.5em] opacity-60 leading-relaxed">
 ...
 <div className="flex items-center gap-8 px-10 py-5 bg-white/[0.02] border border-white/10 rounded-[2.5rem] shadow-inner ">
 <div className="flex items-center gap-3">
 <div className="w-2 h-2 rounded-full bg-[#30D158] shadow-2xl"></div>
 <span className="text-[9px] font-black text-slate-500 uppercase tracking-[0.2em]">0-15m</span>
 </div>
 <div className="flex items-center gap-3">
 <div className="w-2 h-2 rounded-full bg-amber-500 shadow-2xl"></div>
 <span className="text-[9px] font-black text-slate-500 uppercase tracking-[0.2em]">15-30m</span>
 </div>
 <div className="flex items-center gap-3">
 <div className="w-2 h-2 rounded-full bg-rose-500 shadow-2xl"></div>
 ...
 <button
 onClick={() => setEmptyView('WAITING_FORM')}
 className="group relative px-12 py-7 bg-[#0A84FF] hover:bg-indigo-500 text-white rounded-[2.5rem] font-black uppercase tracking-[0.4em] shadow-[0_20px_60px_-10px_rgba(79,70,229,0.5)] transition-all active:scale-95 overflow-hidden"
 >
 ...
 <div className="w-24 h-24 sm:w-32 sm:h-32 bg-[#30D158] rounded-full flex items-center justify-center mx-auto mb-6 animate-bounce shadow-2xl">
 <CheckCircle2 className="w-12 h-12 sm:w-16 sm:h-16 text-white" />
 </div>
 <div className="space-y-4">
 <h2 className="text-4xl sm:text-6xl font-black text-white uppercase tracking-tight ">CONCIERGE READY</h2>
 ...
 return (
 <div className="w-full max-w-lg glass-card p-8 sm:p-14 rounded-[2.5rem] sm:rounded-[4rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-700 z-10 mx-4">
 <div className="absolute top-0 right-0 w-48 h-48 bg-[#0A84FF]/10 blur-[110px]"></div>

 <div className="relative z-10 text-center space-y-4 mb-8 sm:mb-12">
 <div className="w-16 h-16 sm:w-20 sm:h-20 bg-[#0A84FF]/10 rounded-2xl sm:rounded-3xl flex items-center justify-center mx-auto mb-4 border border-[#0A84FF]/20 shadow-2xl">
 <Users className="w-8 h-8 sm:w-10 sm:h-10 text-[#0A84FF]" />
 </div>
 <h2 className="text-2xl sm:text-4xl font-black text-white uppercase tracking-tight ">WAITING LIST</h2>
 ...
 <label className="text-[8px] sm:text-[10px] font-black text-[#0A84FF]/80 uppercase tracking-[0.4em] ml-4 sm:ml-8">Guest Name</label>
 <input
 type="text"
 placeholder="Enter your name..."
 value={waitingName}
 onChange={(e) => setWaitingName(e.target.value)}
 className="w-full px-6 sm:px-10 py-4 sm:py-6 bg-white/[0.03] border border-white/10 rounded-2xl sm:rounded-[2rem] text-white font-black text-lg sm:text-xl focus:border-indigo-600 focus:bg-white/[0.06] outline-none transition-all placeholder:text-slate-800"
 required
 />
 </div>
 <div className="space-y-3 sm:space-y-4">
 <label className="text-[8px] sm:text-[10px] font-black text-[#0A84FF]/80 uppercase tracking-[0.4em] ml-4 sm:ml-8">WhatsApp Number</label>
 <input
 type="tel"
 placeholder="08xxxxxxxxx"
 value={waitingPhone}
 onChange={(e) => setWaitingPhone(e.target.value.replace(/\D/g, ''))}
 className="w-full px-6 sm:px-10 py-4 sm:py-6 bg-white/[0.03] border border-white/10 rounded-2xl sm:rounded-[2rem] text-white font-black text-lg sm:text-xl focus:border-indigo-600 focus:bg-white/[0.06] outline-none transition-all placeholder:text-slate-800 tracking-widest"
 required
 />
 </div>
 ...
 className="order-1 sm:order-2 flex-[2] py-4 sm:py-6 bg-[#0A84FF] hover:bg-indigo-500 disabled:opacity-20 text-white rounded-xl sm:rounded-[1.5rem] font-black uppercase tracking-widest shadow-2lx shadow-indigo-900/40 transition-all flex items-center justify-center gap-3 sm:gap-4 text-[10px] sm:text-[11px]"
 >
 ...
 return (
 <div className="w-full max-w-lg glass-card p-10 rounded-[4rem] shadow-2xl animate-in zoom-in-95 duration-700 z-10 mx-4 relative overflow-hidden">
 <div className="absolute top-0 right-0 w-32 h-32 bg-[#30D158]/10 blur-[80px]" />
 ...
 <Calendar className="w-10 h-10 text-[#30D158]" />
 </div>
 <div>
 <h2 className="text-3xl font-black text-white uppercase tracking-tight ">PRESENSI KARYAWAN</h2>
 ...
 <div className="w-20 h-20 bg-[#30D158] rounded-full flex items-center justify-center mx-auto shadow-2xl">
 <CheckCircle2 className="w-10 h-10 text-white animate-bounce" />
 </div>
 <p className="text-[#30D158] font-black uppercase tracking-widest leading-relaxed text-sm">
 {attendanceMsg}
 </p>
 </motion.div>
 ) : (
 ...
 className="h-16 rounded-2xl bg-white/5 hover:bg-white/10 border border-white/10 text-white font-black text-xl transition-all active:scale-90 shadow-lg"
 >
 {btn}
 </button>
 ))}
 ...
 className="order-2 sm:order-1 flex-1 py-5 bg-white/5 hover:bg-rose-500/10 text-slate-500 hover:text-rose-500 rounded-2xl font-black uppercase tracking-widest text-[9px] transition-all border border-white/10"
 >
 Keluar
 </button>
 ...
 className="flex-1 py-5 bg-emerald-600 hover:bg-[#30D158] disabled:opacity-20 text-white rounded-2xl font-black uppercase tracking-widest text-[9px] shadow-xl shadow-emerald-900/20 transition-all flex items-center justify-center gap-2"
 >
 ...
 className="flex-1 py-5 bg-[#0A84FF] hover:bg-indigo-500 disabled:opacity-20 text-white rounded-2xl font-black uppercase tracking-widest text-[9px] shadow-xl shadow-indigo-900/20 transition-all flex items-center justify-center gap-2"
 >
 ...
 className="fixed inset-0 z-[400] flex items-center justify-center p-4 sm:p-12 bg-[#000000]/95 overflow-y-auto"
 >
 <div className="w-full max-w-6xl min-h-[80vh] bg-white/[0.02] border border-white/10 rounded-[3.5rem] overflow-hidden shadow-2xl flex flex-col relative">
 {/* Header */}
 <header className="p-8 sm:p-12 border-b border-white/10 flex flex-col sm:flex-row justify-between items-center gap-8 bg-gradient-to-br from-amber-600/10 to-orange-600/10">
 <div className="flex items-center gap-6">
 <div className="w-20 h-20 bg-amber-500 rounded-3xl flex items-center justify-center shadow-2xl shadow-amber-500/20">
 <Gift className="w-10 h-10 text-white" />
 </div>
 <div>
 <h1 className="text-3xl sm:text-5xl font-black text-white tracking-tight uppercase leading-none">Redeem Reward</h1>
 ...
 className="w-16 h-16 rounded-[2rem] bg-white/5 hover:bg-rose-500/10 border border-white/10 flex items-center justify-center text-white transition-all group"
 >
 ...
 <div>
 <h3 className="text-xl font-black text-white tracking-tight uppercase mb-2 leading-tight h-12 overflow-hidden">{item.name}</h3>
 ...
 <>
 <div className="space-y-4">
 <h2 className="text-4xl font-black text-white tracking-tight uppercase leading-none">Konfirmasi Penukaran</h2>
 ...
 <div className="bg-white p-8 rounded-[3rem] shadow-2xl inline-block relative group">
 ...
 className="w-full py-4 bg-white/5 text-slate-600 font-black rounded-2xl text-[10px] uppercase tracking-widest border border-white/10 hover:bg-rose-500/10 hover:text-rose-500 transition-all"
 >
 Batalkan Penukaran
 </button>
 </div>
 ...
 >
 <div className="w-32 h-32 bg-[#30D158] rounded-[3rem] flex items-center justify-center mx-auto shadow-[0_30px_60px_-15px_rgba(16,185,129,0.5)]">
 <CheckCircle2 className="w-16 h-16 text-white animate-bounce" />
 </div>
 <div className="space-y-4">
 <h2 className="text-5xl font-black text-white tracking-tight uppercase leading-none">REDEEM BERHASIL!</h2>
 ...
 <span className="text-lg font-black text-white uppercase tracking-tight">{selectedReward.name}</span>
 </div>
 <div className="flex justify-between items-center">
 <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Poin Terpotong</span>
 <span className="text-lg font-black text-amber-400 ">-{selectedReward.pointCost} Pts</span>
 </div>
 </div>
 <p className="text-[10px] font-black text-slate-600 uppercase tracking-[0.5em] ">Terima kasih atas loyalitas Anda.</p>
 </motion.div>
 )}
 </div>
 </motion.div>
 ...
 className="fixed inset-0 z-[400] flex items-center justify-center p-4 sm:p-8 bg-[#000000]/90 "
 >
 <div className="w-full max-w-4xl bg-white/[0.03] border border-white/10 rounded-[3rem] overflow-hidden shadow-2xl relative">
 <div className="p-8 sm:p-12 border-b border-white/10 bg-gradient-to-br from-indigo-600/20 to-purple-600/20 flex flex-col sm:flex-row justify-between items-center gap-8 text-center sm:text-left">
 ...
 <div>
 <p className="text-[#0A84FF] text-xs font-black uppercase tracking-[0.4em] mb-2 ">Member Passport</p>
 <h2 className="text-3xl sm:text-5xl font-black text-white tracking-tight uppercase leading-none">{scannedMember.name}</h2>
 ...
 <div className="p-8 sm:p-12 border-b md:border-b-0 md:border-r border-white/10 space-y-8">
 <div className="grid grid-cols-2 gap-4">
 <div className="bg-white/[0.02] border border-white/10 rounded-3xl p-6 hover:bg-white/[0.04] transition-all group">
 <div className="flex items-center gap-3 mb-4">
 <Wallet className="w-4 h-4 text-[#30D158]" />
 <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">E-Wallet Balance</span>
 </div>
 <p className="text-2xl sm:text-3xl font-black text-[#30D158] tabular-nums">Rp {Number(scannedMember.balance).toLocaleString()}</p>
 </div>
 <div className="bg-white/[0.02] border border-white/10 rounded-3xl p-6 hover:bg-white/[0.04] transition-all group">
 <div className="flex items-center gap-3 mb-4">
 ...
 <div className="bg-[#0A84FF]/10 border border-[#0A84FF]/20 rounded-3xl p-6 space-y-3">
 ...
 <div className="flex items-center gap-3">
 <HistoryIcon className="w-4 h-4 text-[#0A84FF]" />
 <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Recent Activity</span>
 </div>
 <span className="text-[8px] font-black text-slate-600 uppercase tracking-widest ">Last 5 Transactions</span>
 </div>
 ...
 <div key={i} className="bg-white/[0.02] border border-white/10 rounded-2xl p-4 flex justify-between items-center group hover:bg-white/[0.04] transition-all">
 <div>
 ...
 <p className={`font-black text-sm tracking-tight ${log.type === 'TOPUP' ? 'text-[#30D158]' : 'text-rose-400'}`}>
 ...
 return (
 <div className="h-screen w-full bg-[#000000] flex flex-col items-center justify-center text-center relative overflow-hidden select-none">
 <style jsx global>{` 
 body { overflow: hidden; background: #020617; font-family: var(--font-plus-jakarta-sans), sans-serif; } 
 .glass-card { background: rgba(28, 28, 30, 0.7); backdrop-filter: blur(24px); border: 1px solid rgba(255, 255, 255, 0.1); }
 ...
 >
 <div className="bg-[#30D158] rounded-[2.5rem] p-10 shadow-[0_30px_60px_-15px_rgba(16,185,129,0.5)] border border-white/20 relative overflow-hidden group">
 <motion.div
 ...
 <div>
 <h3 className="text-3xl font-black text-white tracking-tight uppercase leading-none">Topup Success!</h3>
 ...
 <p className="text-4xl font-black text-white tabular-nums tracking-tight">Rp {topupSuccess.newBalance.toLocaleString()}</p>
 </div>

 <div className="pt-2">
 <p className="text-[8px] font-black text-white/40 uppercase tracking-[0.5em] ">Pentagon Billiard Loyalty & System</p>
 </div>
 </div>
 </div>
 </motion.div>
 ...
 <div className="w-14 h-14 bg-[#30D158] rounded-2xl flex items-center justify-center shadow-lg shadow-emerald-500/20">
 <CheckCircle2 className="w-8 h-8 text-white" />
 </div>
 <div className="text-left">
 <p className="text-[10px] font-black text-[#30D158] uppercase tracking-widest mb-1">Scan Berhasil</p>
 <h4 className="text-xl font-black text-white leading-tight uppercase tracking-tight ">Member: {validationAlert.name}</h4>
 <div className="flex items-center gap-2 mt-1">
 <div className={`w-2 h-2 rounded-full ${validationAlert.active ? 'bg-[#30D158] animate-pulse' : 'bg-rose-500'}`} />
 ...
 initial={{ opacity: 0, scale: 0.95 }}
 animate={{ opacity: 1, scale: 1 }}
 className="fixed inset-0 z-[100] bg-[#000000] flex flex-col items-center justify-center p-8 sm:p-20"
 >
 <BackgroundAnimation />
 <div className="relative z-10 w-full max-w-4xl space-y-12">
 <div className="text-center space-y-4">
 <div className="w-20 h-20 bg-indigo-500/10 rounded-[2rem] flex items-center justify-center mx-auto mb-6 border border-[#0A84FF]/20 shadow-2xl">
 <Monitor className="w-10 h-10 text-[#0A84FF]" />
 </div>
 <h1 className="text-4xl sm:text-6xl font-black text-white uppercase tracking-tight ">Terminal Setup</h1>
 ...
 <div className="space-y-4">
 <p className="text-[10px] font-black text-[#0A84FF] uppercase tracking-[0.4em] ml-2">Quick Locations</p>
 ...
 className="px-6 py-4 bg-[#0A84FF]/10 hover:bg-[#0A84FF]/20 border border-[#0A84FF]/20 hover:border-indigo-500/40 rounded-2xl text-white font-black text-sm transition-all active:scale-95 flex items-center justify-center gap-3 group"
 >
 <Zap className="w-4 h-4 text-[#0A84FF] group-hover:animate-pulse" />
 DISPLAY-{loc}
 </button>
 ))}
 </div>
 ...
 className="px-4 py-3 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl text-white font-black text-sm transition-all active:scale-95 flex items-center justify-center gap-1 group"
 >
 ...
 <Monitor className="absolute left-6 top-1/2 -translate-y-1/2 w-5 h-5 text-[#0A84FF]" />
 <input
 type="text"
 ...
 className="px-10 py-5 bg-[#0A84FF] text-white rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-indigo-500 transition-all shadow-lg shadow-indigo-900/40"
 >
 Connect
 </button>
 </div>
 ...
 <div className="w-3 h-3 bg-rose-500 rounded-full shadow-2xl"></div>
 <span className="text-[11px] font-black text-rose-500 uppercase tracking-[0.5em] ">Connecting...</span>
 </div>
 </div>
 )}
 ...
 <div className="w-8 h-8 sm:w-10 sm:h-10 bg-indigo-500 rounded-full flex items-center justify-center p-1.5 shadow-2xl">
 {settings?.logoPath ? (
 ...
 <h2 className="text-lg sm:text-2xl font-black text-white uppercase tracking-tight text-right">
 {settings?.businessName || "PAKTEKA 88"}
 </h2>
 </div>

 <div className="flex-1 flex flex-col md:flex-row items-center justify-between gap-12 sm:gap-20">
 {/* LEFT SIDE: PROMO HEADING & LARGE HERO IMAGE */}
 <div className="flex-1 flex flex-col items-start gap-8 sm:gap-12 w-full md:max-w-[800px]">
 ...
 className="space-y-2"
 >
 <h1 className="text-5xl sm:text-7xl md:text-8xl font-black text-white uppercase tracking-tight leading-none">
 {currentPromo.title}
 </h1>
 ...
 className="absolute inset-0 rounded-[2.5rem] sm:rounded-[3.5rem] overflow-hidden border border-white/10 shadow-[0_60px_120px_-20px_rgba(0,0,0,0.8)] bg-slate-900"
 >
 {currentPromo?.image ? (
 <img
 ...
 {settings ? (
 <Zap className="w-32 h-32 text-[#0A84FF] opacity-20 animate-pulse" />
 ) : (
 ...
 <div className="w-full md:w-[400px] flex flex-col items-center md:items-center gap-10">
 <div className="text-center md:text-right space-y-2">
 <p className="text-3xl sm:text-5xl font-black tracking-tight leading-none flex flex-wrap justify-center md:justify-end gap-3 uppercase">
 <span className="text-[#0A84FF]">CEK</span>
 ...
 <button
 onClick={() => setEmptyView('WAITING_FORM')}
 className="flex-1 px-8 py-6 bg-[#0A84FF] text-white rounded-2xl font-black uppercase tracking-widest text-sm shadow-[0_20px_40px_-10px_rgba(79,70,229,0.4)] hover:bg-indigo-500 transition-all active:scale-95 flex items-center justify-center gap-4 group"
 >
 ...
 className="flex-1 px-8 py-5 bg-emerald-600/10 hover:bg-emerald-600/20 border border-emerald-500/20 rounded-2xl text-[#30D158] font-black uppercase tracking-widest text-xs transition-all active:scale-95 flex items-center justify-center gap-4"
 >
 ...
 <span className="text-[10px] sm:text-xs font-black text-[#0A84FF] uppercase tracking-[0.3em] ">
 ...
 {emptyView === 'PROMO' && (
 <div className="px-6 py-4 flex flex-col sm:flex-row justify-between items-center bg-[#000000] border-t border-white/10 relative z-50 gap-4">
 ...
 className="px-4 py-2 bg-white/5 hover:bg-rose-500/10 border border-white/10 hover:border-rose-500/20 rounded-lg text-[8px] font-black text-slate-700 hover:text-rose-500 uppercase tracking-widest transition-all"
 >
 Reset Terminal Configuration
 </button>
 <div className="flex items-center gap-2">
 <div className="w-1.5 h-1.5 bg-[#30D158] rounded-full shadow-2xl"></div>
 <span className="text-[7px] font-black text-slate-700 uppercase ">{terminalId} Connected</span>
 </div>
 </div>
 </div>
 )}
 ...
 if (showRating) {
 return (
 <div className="h-screen w-screen bg-[#0A84FF] flex flex-col items-center justify-center p-12 text-center text-white select-none relative overflow-hidden">
 ...
 <div className="space-y-4">
 <h1 className="text-5xl font-black tracking-tight uppercase">Terima Kasih!</h1>
 ...
 return (
 <div className="h-[100dvh] w-screen bg-[#000000] flex flex-col select-none relative overflow-hidden text-white font-sans">
 <style jsx global>{` 
 :root { --p-bg: #020617; }
 body { overflow: hidden; background: var(--p-bg); font-family: var(--font-plus-jakarta-sans), sans-serif; color: white; } 
 ::-webkit-scrollbar { display: none; } 
 .glass-card { background: rgba(28, 28, 30, 0.7); backdrop-filter: blur(24px); border: 1px solid rgba(255, 255, 255, 0.1); box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.5); }
 ...
 disabled={isCalling}
 className={`px-6 py-4 rounded-2xl font-black text-[9px] uppercase tracking-widest flex items-center gap-4 shadow-2xl transition-all border ${isCalling ? 'bg-[#30D158] text-white border-emerald-400' : 'bg-white/5 text-white border-white/10 hover:bg-white/10'}`}
 >
 <BellRing className={`w-4 h-4 ${isCalling ? 'animate-bounce' : 'text-[#0A84FF]'}`} />
 {isCalling ? 'Waiter Notified' : 'Panggil Waiter'}
 </button>
 </div>

 <div className="h-24 px-10 flex justify-between items-center border-b border-white/10 relative z-50 bg-[#000000]/60 shadow-2xl">
 <div className="flex flex-col text-left">
 <span className="text-base font-black tracking-tight text-white uppercase leading-none">{settings?.businessName || "VOC SYSTEM PREMIUM"}</span>
 <div className="flex items-center gap-2 mt-2">
 <div className="w-1.5 h-1.5 bg-indigo-500 rounded-full shadow-2xl"></div>
 <span className="text-[9px] font-black tracking-[0.4em] text-slate-500 uppercase ">
 ...
 <div className="flex flex-col items-center gap-1">
 <p className="text-[9px] font-black text-slate-600 uppercase tracking-[0.3em] ">Accessing</p>
 <span className="px-5 py-2 bg-[#0A84FF]/10 border border-[#0A84FF]/20 text-[#0A84FF] rounded-2xl text-[11px] font-black uppercase tracking-tight shadow-inner">
 {table?.tableName || tx?.invoiceNumber || 'GUEST-01'}
 </span>
 </div>
 <div className="w-12 h-12 bg-white/5 rounded-[1.25rem] border border-white/10 flex items-center justify-center overflow-hidden p-2 shadow-2xl">
 {settings?.logoPath ? (
 ...
 <div className="w-full md:w-[58%] h-auto md:h-full p-4 md:p-6 lg:p-10 flex flex-col gap-4 md:gap-8 border-b md:border-b-0 md:border-r border-white/10 overflow-visible md:overflow-hidden relative">
 <div className="flex items-center justify-between relative z-10">
 <div className="space-y-1 sm:space-y-2">
 <p className="text-[8px] sm:text-[10px] text-[#0A84FF] font-black uppercase tracking-[0.5em]">Current Session</p>
 <h3 className="text-2xl sm:text-4xl font-black text-white uppercase tracking-tight leading-none truncate max-w-[180px] sm:max-w-none">
 {paymentState?.customerName || member?.name || tx?.member?.name || tx?.customerName || 'Walk-in Guest'}
 </h3>
 </div>
 <div className={`px-4 sm:px-6 py-2 sm:py-2.5 rounded-xl sm:rounded-2xl flex items-center gap-2 sm:gap-3 glass-card border ${tx?.isPaid ? 'border-[#30D158]/30 text-[#30D158]' : 'border-amber-500/30 text-amber-400'}`}>
 <div className={`w-1.5 sm:w-2 h-1.5 sm:h-2 rounded-full ${tx?.isPaid ? 'bg-[#30D158] shadow-2xl' : 'bg-amber-500 animate-pulse shadow-2xl'}`}></div>
 <span className="text-[8px] sm:text-[10px] font-black uppercase tracking-widest">{tx?.isPaid ? 'Paid' : 'Unpaid'}</span>
 </div>
 </div>

 <div className="flex-1 space-y-4 flex flex-col relative z-10 md:overflow-hidden">
 <div className="flex-1 md:overflow-y-auto noscrollbar space-y-3">
 ...
 <div className="w-10 h-10 sm:w-14 sm:h-14 bg-indigo-500/10 text-[#0A84FF] rounded-xl sm:rounded-2xl flex items-center justify-center shadow-inner"><Timer className="w-5 h-5 sm:w-6 sm:h-6" /></div>
 <div className="space-y-0.5 sm:space-y-1">
 <p className="font-black text-white text-sm sm:text-lg uppercase tracking-tight ">{tx.fareName || 'Billiard Session'}</p>
 ...
 <p className="text-lg sm:text-2xl font-black text-white font-mono tracking-tight">Rp {Number(tx.billiardTotal).toLocaleString()}</p>
 </div>
 )}
 ...
 <p className="font-bold text-slate-200 text-xs sm:text-sm uppercase tracking-wide truncate max-w-[140px] sm:max-w-none">{item.menuItem?.name || 'Cafe Order'}</p>
 <p className="text-slate-600 font-black text-[8px] sm:text-[9px] uppercase tracking-widest">{Number(item.quantity)}x @ {item.priceAtOrder.toLocaleString()}</p>
 </div>
 </div>
 <p className="font-black text-slate-300 text-sm sm:text-lg font-mono tracking-tight">Rp {Number(item.priceAtOrder * item.quantity).toLocaleString()}</p>
 </div>
 ))}
 </div>
 </div>
 ...
 <div className="w-full md:w-[42%] h-auto md:h-full bg-[#030712]/40 flex flex-col p-4 md:p-6 lg:p-10 gap-4 md:gap-8 relative overflow-y-auto noscrollbar">
 {/* Summary Card */}
 ...
 <div className="glass-card rounded-[2rem] lg:rounded-[2.5rem] p-5 md:p-6 lg:p-8 space-y-4 lg:space-y-6 relative z-10 overflow-hidden shrink-0">
 {tx?.isPaid && (
 <div className="absolute top-4 right-4 z-20">
 <span className="px-3 py-1 bg-[#30D158] text-white text-[8px] font-black uppercase tracking-widest rounded-full shadow-2xl">SETTLED</span>
 </div>
 )}
 <div className="space-y-3 sm:space-y-4">
 ...
 <div className="flex justify-between items-center text-[8px] sm:text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] border-b border-white/10 pb-3 sm:pb-5">
 ...
 <p className="text-[8px] sm:text-[9px] font-black uppercase tracking-[0.5em] text-[#0A84FF] ">Grand Total</p>
 <div className="flex items-baseline gap-2 sm:gap-3">
 <span className="text-sm sm:text-xl font-black text-white/10 ">IDR</span>
 <h1 className={`text-3xl sm:text-6xl font-black tracking-tight origin-left transition-colors ${tx?.isPaid ? 'text-[#30D158]' : 'text-white'}`}>
 ...
 <div className="flex-1 md:overflow-hidden min-h-[350px] md:min-h-0">
 <AnimatePresence mode="wait">
 ...
 <div className="p-6 sm:p-8 bg-[#0A84FF] rounded-[2rem] text-white shadow-2xl h-full flex flex-col">
 <div className="flex items-center gap-3 mb-4">
 ...
 <div className="w-full max-w-[380px] mx-auto flex flex-col items-center gap-2 sm:gap-3 bg-white/5 p-4 rounded-2xl border border-white/10 shadow-2xl">
 <p className="text-[9px] font-black text-white/50 uppercase tracking-[0.2em]">Metode Pembayaran Didukung</p>
 <div className="flex flex-wrap justify-center gap-1.5 sm:gap-2">
 <span className="px-2 py-0.5 bg-white rounded text-[9px] font-black text-[#005E6A] shadow-sm">Mandiri</span>
 <span className="px-2 py-0.5 bg-white rounded text-[9px] font-black text-[#0066AE] shadow-sm">BCA</span>
 ...
 <div className="mt-4 flex-1 overflow-y-auto noscrollbar bg-black/10 rounded-xl p-3 border border-white/10">
 ...
 <p className="text-slate-500 text-[6px] sm:text-[8px] font-bold uppercase tracking-widest mt-1 opacity-60 ">E-Wallet & Mobile Banking Ready</p>
 </div>
 </div>
 </motion.div>
 )}
 ...
 <div className="px-6 sm:px-10 py-3 flex flex-col sm:flex-row justify-between items-center bg-[#000000] border-t border-white/10 relative z-50 gap-4">
 ...
 <div className="flex items-center gap-2">
 <div className="w-1.5 h-1.5 bg-[#30D158] rounded-full shadow-2xl"></div>
 <span className="text-[7px] font-black text-slate-700 uppercase ">{terminalId} Active</span>
 </div>
 </div>
 </div>
 
 {/* RATING POPUP MODAL (ULTRA SIMPLE iOS) */}
        <AnimatePresence>
            {paymentState && showRatingPopup && (
                <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-black/80 p-4">
                    {!ratingSubmitted ? (
                        <div className="bg-[#1C1C1E] border border-white/10 p-8 rounded-[2rem] w-full max-w-sm shadow-2xl flex flex-col items-center animate-in fade-in zoom-in duration-300">
                            <h2 className="text-white text-xl font-bold mb-1 text-center">
                                Halo kak {paymentState?.customerName || member?.name || tx?.member?.name || tx?.customerName || ''}! 👋
                            </h2>
                            <p className="text-[#EBEBF5]/60 text-xs mb-8 text-center">Bantu nilai pelayanan kami hari ini yuk!</p>

                            <div className="w-full space-y-6">
                                {/* Waiter Rating */}
                                <div className="flex flex-col items-center">
                                    <p className="text-[10px] font-semibold text-[#EBEBF5]/60 uppercase tracking-widest mb-3">Waiter: {tx?.commissionUser?.name || tx?.openedBy?.name || 'Staff'}</p>
                                    <div className="flex justify-center gap-4">
                                        {[1, 2, 3, 4, 5].map((star) => (
                                            <button 
                                                key={'w'+star}
                                            ...
                                            >
                                                <Star className={`w-8 h-8 ${waiterRating >= star ? 'fill-[#FFD60A] text-[#FFD60A]' : 'fill-[#3A3A3C] text-[#3A3A3C]'}`} />
                                            </button>
                                        ))}
                                    </div>
                                </div>
                                ...
                                {/* Kasir Rating */}
                                <div className="flex flex-col items-center">
                                    <p className="text-[10px] font-semibold text-[#EBEBF5]/60 uppercase tracking-widest mb-3">Kasir: {paymentState?.cashierName || tx?.createdBy?.name || 'Kasir'}</p>
                                    <div className="flex justify-center gap-4">
                                        {[1, 2, 3, 4, 5].map((star) => (
                                            <button 
                                                key={'k'+star}
                                                ...
                                                className="focus:outline-none transition-transform active:scale-90"
                                            >
                                                <Star className={`w-8 h-8 ${kasirRating >= star ? 'fill-[#FFD60A] text-[#FFD60A]' : 'fill-[#3A3A3C] text-[#3A3A3C]'}`} />
                                            </button>
                                        ))}
                                    </div>
                                </div>
                                ...
                                <button
                                    onClick={async () => {
                                        try {
                                            ...
                                            // Fallback: forcefully close it if API fails so they aren't trapped
                                            setShowRatingPopup(false); 
                                        }
                                    }}
                                    className={`w-full py-4 rounded-xl font-semibold text-sm transition-colors text-white ${waiterRating === 0 && kasirRating === 0 ? 'bg-[#3A3A3C] active:bg-[#2C2C2E]' : 'bg-[#0A84FF] active:bg-[#086dd3]'}`}
                                >
                                    {waiterRating === 0 && kasirRating === 0 ? 'Lewati' : 'Kirim'}
                                </button>
                            </div>
                        </div>
                    ) : (
                        <div className="bg-[#1C1C1E] border border-white/10 p-8 rounded-[2rem] w-full max-w-sm shadow-2xl flex flex-col items-center justify-center text-center animate-in fade-in zoom-in duration-300">
                            <div className="w-16 h-16 bg-[#30D158]/20 rounded-full flex items-center justify-center mb-4">
                                <CheckCircle2 className="w-8 h-8 text-[#30D158]" />
                            </div>
                            <h3 className="text-xl font-bold text-white mb-2">Terima Kasih!</h3>
                            <p className="text-xs text-[#EBEBF5]/60">Penilaian Anda telah tersimpan.</p>
                        </div>
                    )}
                </div>
            )}
