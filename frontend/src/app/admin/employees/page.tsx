...
export default function EmployeePage() {
    ...
    const handleExportExcel = () => {
        try {
            ...
            // 1. Sheet Role Matrix — export data aktual dari database
            const roleData = [
            ...
            // 2. Sheet Karyawan — export data aktual semua karyawan
            const empHeader = [
            ...
            // 3. Sheet Panduan — referensi permissions akurat sesuai sistem
            const guideData = [
                ['PANDUAN IMPORT/EDIT DATA SDM - Billiard Management System'],
                [''],
                ['CARA PENGGUNAAN:'],
                ['1. Sheet "Role Matrix" berisi semua role beserta permissions aktual dari sistem.'],
                ['2. Sheet "Karyawan" berisi data semua karyawan. Kolom Password dikosongkan (isi jika ingin ganti password).'],
                ['3. Untuk mengganti password massal: isi kolom Password, lalu Import kembali file ini.'],
                ['4. Setelah Import, karyawan harus Logout & Login ulang agar perubahan aktif.'],
                [''],
                ['━━━ DAFTAR SEMUA PERMISSION YANG TERSEDIA ━━━'],
                [''],
                ['[ Dashboard & Analytics ]'],
                ['Permission ID', 'Keterangan'],
                ...
                ['MEMBER_MANAGE', 'Tambah/Edit/Hapus Member & Tier'],
                ...
                ['INV_TOGGLE_MENU', 'Aktif / Non-Aktifkan Menu'],
                ['INVENTORY_STOCK_IN', 'Penerimaan / Tambah Stok (+)'],
                ['INVENTORY_STOCK_OUT', 'Pemakaian / Kurangi Stok (-)'],
                ['INVENTORY_WASTE', 'Deklarasi Waste / Pembuangan'],
                ['INVENTORY_STOCK_ADJUST', 'Adjustment / Koreksi Stok'],
                ['INVENTORY_SUPPLIER_MANAGE', 'Manajemen Data Supplier'],
                ['STOCK_OPNAME', 'Akses Menu Stock Opname'],
                ['INV_EXPORT', 'Export Data Inventory (Excel)'],
                [''],
                ['[ Keuangan, Piutang & Laporan ]'],
                ...
                ['FIN_PRINT_REPRINT', 'Cetak Ulang Invoice Lama'],
                ...
                ['AR_LIST_VIEW', 'Lihat Daftar Piutang (AR)'],
                ['AR_PAYMENT', 'Proses Pembayaran Piutang'],
                ['AR_SETTLE', 'Finalisasi Pelunasan Piutang'],
                ['SHIFT_REPORT', 'Lihat Laporan Detail Shift'],
                [''],
                ['[ SDM, Audit & Keamanan ]'],
                ['USER_MANAGE', 'Kelola Akun & Hak Akses'],
                ['USER_EDIT', 'Edit Profil & Data Karyawan'],
                ['USER_DELETE', 'Hapus Data Karyawan'],
                ['USER_VIOLATION', 'Input Pelanggaran (Log Violation)'],
                ['USER_ROLE', 'Konfigurasi Role & Matrix'],
                ['USER_MONITOR', 'Monitor Aktivitas (Audit Trail)'],
                ['USER_FORCE_LOGOUT', 'Paksa Logout Sesi Aktif'],
                ['USER_EXPORT', 'Export Data Karyawan (Excel)'],
                ['AUDIT_VIEW', 'Lihat Audit Log Aktivitas'],
                ['AUDIT_EXPORT', 'Export Data Audit Trail'],
                ['PAYROLL_VIEW', 'Lihat Laporan Gaji & Komisi'],
                ['SHIFT_START', 'Mulai Shift (Buka Kasir)'],
                ['SHIFT_MANAGE', 'Manajemen / Edit Shift'],
                ['APPROVAL_OVERRIDE', 'Otoritas Bypass / Super-User'],
                ...
                ['TABLE_CREATE', 'Tambah Meja Baru (Billiard & Cafe)'],
                ...
                ['SETTING_POLICY', 'Atur Pajak, Biaya & Pembulatan'],
                ['SETTING_HARDWARE', 'Konfigurasi IoT, IP & Printer'],
                ['SETTING_INVOICE', 'Kustomisasi Invoice'],
                ['SETTING_LAYOUT', 'Desain Layout Ruangan'],
                ['SETTING_DISPLAY', 'Display & Digital Marketing'],
                ['SETTING_GAMIFICATION', 'Gamifikasi & Poin'],
                ['PROMO_MANAGE', 'Kelola Promo & Bundling'],
                ['LOCKER_MANAGE', 'Manajemen Locker'],
                ['VOUCHER_MANAGE', 'Manajemen Master Voucher'],
                [''],
                ['CATATAN PENTING:'],
                ['- Kolom Password di sheet Karyawan dikosongkan karena alasan keamanan.'],
                ['- Isi kolom Password jika ingin mengganti/mereset password karyawan, lalu import ulang.'],
                ['- Untuk role: edit kolom Permissions di sheet Role Matrix, lalu import ulang.'],
                ['- Setelah import selesai, karyawan harus Logout & Login ulang agar perubahan aktif.'],
            ];
            const wsGuide = xlsx.utils.aoa_to_sheet(guideData);
            wsGuide['!cols'] = [{ wch: 30 }, { wch: 85 }, { wch: 12 }];
