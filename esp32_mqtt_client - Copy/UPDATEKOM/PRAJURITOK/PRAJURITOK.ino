...
// State & Config
int32_t currentMesaId = 4; // <--- GANTI DISINI (ID Meja / relayPin)
uint8_t CURRENT_COMMANDER_MAC[6] = {0, 0, 0, 0, 0, 0};
// MAC KOMANDAN (Opsional: Isi MAC Komandan Anda di sini agar pairing instant)
// Contoh: {0x20, 0x6E, 0xF1, 0x70, 0x81, 0xE8}
uint8_t HARDCODED_COMMANDER_MAC[6] = {0x70, 0x4b, 0xca, 0x8f, 0x72, 0x54};
...
void setLight(bool on) {
  if (isLightOn == on)
    return; // ABAIKAN JIKA STATUS SAMA (Cegah NVS Wear & Spam)
  isLightOn = on;
...
void lockCommander(const uint8_t *src_addr) {
  if (memcmp(CURRENT_COMMANDER_MAC, src_addr, 6) != 0 || !hasCommander) {
...
void OnDataRecv(const esp_now_recv_info_t *info, const uint8_t *incomingData,
  ...
  if (temp.mesaId == 0) {
    ...
    if (ch != currentSavedCh && ch >= 1 && ch <= 13) {
      ...
      // SINKRONISASI MUTLAK: Geser radio fisik Prajurit ke channel asli
      // Komandan SEKARANG!
...
void setup() {
  ...
  bool foundInNvs = (prefs.getBytes("commMac", CURRENT_COMMANDER_MAC, 6) == 6);

  // Jika NVS kosong tapi ada HARDCODED_MAC, pakai yang hardcoded
  bool hasHardcoded = false;
  for (int i = 0; i < 6; i++) {
    if (HARDCODED_COMMANDER_MAC[i] != 0)
      hasHardcoded = true;
  }

  if (!foundInNvs && hasHardcoded) {
    memcpy(CURRENT_COMMANDER_MAC, HARDCODED_COMMANDER_MAC, 6);
    foundInNvs = true;
  }
...
void loop() {
  ...
  // 0. Channel Hopping dengan Active Discovery (Super Cepat)
  if (!hasCommander) {
    static unsigned long lastHop = 0;
    if (now - lastHop > 300) { // Hop setiap 300ms (Sangat Cepat)
      lastHop = now;
      currentSavedCh++;
      ...
      // KIRIM SINYAL DISCOVERY (Ketuk Pintu)
      struct_message disc;
      disc.mesaId = currentMesaId;
      disc.cmd = 99; // Kode Discovery
      disc.wifiChannel = currentSavedCh;
      uint8_t broadcastMAC[] = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF};
      esp_now_send(broadcastMAC, (uint8_t *)&disc, sizeof(disc));

      Serial.printf("[SCAN] Ch: %d | Mengetuk pintu Komandan...\n",
                    currentSavedCh);
    }
  }

  // 0.1 DARURAT: Jika sudah punya Komandan tapi tidak mendengar suara Komandan 
  // selama 15 detik, maka masuk ke mode Discovery lagi (Cari Channel).
  if (hasCommander && (now - lastHeardCommander > 15000)) {
    Serial.printf("[SYSTEM] Kehilangan sinyal Komandan. Mencari ulang...\n");
    hasCommander = false; 
  }
  ...
  if (hasNewCommand) {
    ...
    // Tutup mulut selama 3 detik agar perintah tidak bertabrakan dengan
    // Heartbeat
    silenceUntil = millis() + 3000;
  }
