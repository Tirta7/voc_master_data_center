...
void callback(char *topic, byte *payload, unsigned int length) {
  ...
  if (sTopic.endsWith("/light/set")) {
    ...
    if (activate) {
      // Jika ada duration (Prepaid) -> Set Timer. Jika tidak ada (Open Table) -> Reset ke 0
...
void setup() {
  ...
  if (lightState) {
    Serial.println(
        "[HARDWARE] ♻ POWER RESTORE: Lampu MENYALA (state dari SPIFFS)");
...
void loop() {
  ...
  if (currentBtnState == LOW && lastBtnState == HIGH &&
    ...
    lastStateChange = now;
    startBuzzer(150);
    Serial.printf("[HARDWARE] Tombol ditekan! Mode: MANUAL, State: %s\n",
                  lightState ? "ON" : "OFF");
    publishStatus(); // Kirim status segera agar backend tahu ini manual
  }
  ...
  if (lightState && failsafeSeconds > 0) {
    if (now - lastFailsafeTick >= 1000) {
      ...
      if (failsafeSeconds % 30 == 0) storageDirty = true;

      if (failsafeSeconds == 0) {
        Serial.println("[TIMER] ⏰ Failsafe Triggered! Mematikan lampu mandiri...");
        setLight(false);
