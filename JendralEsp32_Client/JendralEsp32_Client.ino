...
String knownMac = ""; // MAC Address panel yang ditemukan
        ...
        for (int i = 0; i < relays.size(); i++) {
          ...
          if (tableId > 0 && tableId <= 100 && freeTables[tableId]) {
            ...
            if (!ts.isOn && st) {
              ...
              // Meja dimatikan/dibayar via Aplikasi Kasir
              finalSt = false;
              kasirOff = true;
            }
          }
...
void handleRoot() {
  ...
  html += "<div class='status'><div class='dot " +
          String((knownMac != "") ? "on" : "") + "'></div>";
  if (knownMac != "")
    html += "MQTT SYNC: " + knownMac;
  else
    html += "WAITING MQTT...";
  html += "</div></header>";
  ...
  html += "    var isKasir = (data.pkg === 'APP KASIR');\n";
  html += "    if (btnOn) {\n";
  html += "      btnOn.textContent = isKasir ? 'LOCKED' : 'EXTEND';\n";
  html += "      if (isKasir) {\n";
  html += "         btnOn.classList.add('btn-dim');\n";
  html += "         btnOn.onclick = null;\n";
  html += "      } else {\n";
  ...
  html += "    }\n";
  html += "    if (btnOff) {\n";
  html += "      btnOff.textContent = isKasir ? 'LOCKED' : 'STOP';\n";
  html += "      if (isKasir) {\n";
  html += "         btnOff.classList.add('btn-dim');\n";
  html += "         btnOff.onclick = null;\n";
  html += "      } else {\n";
...
void setup() {
  ...
  if (prefs.getBytes("cfg", &cfg, sizeof(cfg)) != sizeof(cfg)) {
    ...
    strcpy(cfg.ssid, "");
    strcpy(cfg.wifiPass, "");
    strcpy(cfg.mqttServer, "192.168.1.5");
    cfg.mqttPort = 1883;
    prefs.putBytes("cfg", &cfg, sizeof(cfg));
  }
...
void BackgroundLoop(void *pvParameters) {
  for (;;) {
    ...
    if (now - lastBlink > (mqtt.connected() ? 1000 : 200)) {
      lastBlink = now;
