 ...
 * ║  Chip       : ESP8266 (ESP-12F / NodeMCU / Wemos)                ║
 * ╚══════════════════════════════════════════════════════════════════╝
 */
...
// ─── PINS ────────────────────────────────────────────────────────
// Penyesuaian pin untuk ESP8266 (NodeMCU / Wemos)
#define PIN_RELAY 5 // D1 pada NodeMCU/Wemos
#define PIN_LED 2    // D4 pada NodeMCU/Wemos (Built-in LED, Active Low)
#define PIN_BOOT 0   // D3 pada NodeMCU/Wemos (Tombol FLASH bawaan board)
...
// Struktur gabungan untuk disimpan di EEPROM
struct EepromData {
  uint32_t magic;
  PrajuritConfig cfg;
  bool state;
...
// Alias agar tidak merombak kode utama terlalu banyak
#define cfg savedData.cfg

void loadConfig() {
  EEPROM.begin(EEPROM_SIZE);
  EEPROM.get(0, savedData);
  if (savedData.magic != EEPROM_MAGIC) {
    // Format if empty or corrupted
...
void lockCommander(const uint8_t *srcMac, int ch) {
  ...
  if (!hasCommander || channelChanged) {
    ...
    esp_now_add_peer(cmdMacBytes, ESP_NOW_ROLE_COMBO, ch, NULL, 0);
    
    // 🛡️ FORCE PHYSICAL SWITCH (ESP8266 version)
    wifi_promiscuous_enable(1);
    wifi_set_channel(ch);
    wifi_promiscuous_enable(0);
...
// ─── ESP-NOW RECEIVE (ESP8266 Version) ───────────────────────────
void OnDataRecv(uint8_t *mac_addr, uint8_t *data, uint8_t len) {
  ...
  if (remoteCh >= 1 && remoteCh <= 13) {
    lockCommander(mac_addr, remoteCh);
  }
...
<title>PRAJURIT CONFIG (ESP8266) — VOC BILLIARD</title>
...
void setup() {
  ...
  // 🛡️ AMANKAN RELAY: Paksa HIGH (OFF) SEBELUM jadi OUTPUT
  digitalWrite(PIN_RELAY, HIGH); 
  pinMode(PIN_RELAY, OUTPUT);
  digitalWrite(PIN_RELAY, HIGH); 

  pinMode(PIN_LED, OUTPUT);
  digitalWrite(PIN_LED, HIGH); // LED built-in ESP8266 mati saat HIGH
  ...
  bool hasConfig = (savedData.magic == EEPROM_MAGIC);
  ...
  Serial.printf(
      "\n[SYSTEM] Prajurit ESP8266 Meja %d | Komandan: %s | Ch Tersimpan: %d\n",
      cfg.mesa_id, cfg.commander_mac, cfg.saved_channel);

  // Power Recovery
  if (savedData.state) {
    int rem = savedData.remMin;
    setLight(true); 
    if (rem > 0) {
  ...
  WiFi.mode(WIFI_STA);
  // Matikan power saving di ESP8266 agar tidak lag
  ...
  // Broadcast peer
  uint8_t bc[] = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF};
  esp_now_add_peer(bc, ESP_NOW_ROLE_COMBO, 1, NULL, 0);

  // Unicast peer
  if (isMacSet(cmdMacBytes)) {
    esp_now_add_peer(cmdMacBytes, ESP_NOW_ROLE_COMBO, 1, NULL, 0);
  }
...
void loop() {
  ...
  if (hasNewCommand) {
    ...
    if (cmd.token != 0 && cmd.token == lastToken) {
      ...
      if (isOn) {
        ...
        setLight(true);
        savedData.remMin = cmd.durationMin;
        saveConfig();
      } else if (isOff) {
        setLight(false);
      }
  ...
  if (hasCommander && now > silenceUntil && now - lastHb > HEARTBEAT_INTERVAL_MS) {
    ...
    if (isLightOn && autoOffAt > now) {
      ...
      if (currentRem != savedData.remMin) {
        savedData.remMin = currentRem;
        saveConfig();
      }
    }
