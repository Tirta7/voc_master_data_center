...
void OnDataRecv(const esp_now_recv_info_t *info, const uint8_t *incomingData,
  ...
  if (temp.mesaId == 0) {
    lockCommander(
        info->src_addr); // Selalu panggil, memcmp akan memfilter duplikat
...
void loop() {
  ...
  if (requestSaveChannel) {
    ...
    prefs.putBytes("cmdMac", CURRENT_COMMANDER_MAC, 6);
    prefs.putInt("cmdCh", WiFi.channel());
    Serial.printf("[SYSTEM] Komandan Terkunci & Tersimpan di Channel: %d\n", WiFi.channel());
  }
