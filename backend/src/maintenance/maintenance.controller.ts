...
export class MaintenanceController {
    ...
    async previewMaintenance(
        @Query('auditLogDays', new DefaultValuePipe(30), ParseIntPipe) auditLogDays: number,
        @Query('sessionDays', new DefaultValuePipe(90), ParseIntPipe) sessionDays: number,
        @Query('transactionDays', new DefaultValuePipe(90), ParseIntPipe) transactionDays: number,
        @Query('cashflowDays', new DefaultValuePipe(365), ParseIntPipe) cashflowDays: number,
    ) {
        return this.maintenanceService.getPreviewCounts({ auditLogDays, sessionDays, transactionDays, cashflowDays });
    }
    ...
    async runMaintenance() {
        ...
        return { message: 'Maintenance selesai dijalankan. Cek server logs untuk detail.' };
    }
    ...
    async purgeAuditLogs(
        ...
        return { message: `Berhasil menghapus ${count} audit log lebih dari ${days} hari` };
    }
    ...
    async purgeSessions(
        ...
        return { message: `Berhasil menghapus ${count} session lebih dari ${days} hari` };
    }
    ...
    async archiveTransactions(
        ...
        return { message: `Berhasil mengarsipkan ${count} transaksi lebih dari ${days} hari` };
    }
    ...
    async archiveCashflow(
        ...
        return { message: `Berhasil mengarsipkan ${count} cashflow lebih dari ${days} hari` };
    }
