...
export class LoyaltyService implements OnModuleInit {
  ...
  constructor(
    @InjectRepository(PointReward) private rewardRepo: Repository<PointReward>,
    @InjectRepository(PointLedger) private ledgerRepo: Repository<PointLedger>,
    @InjectRepository(Member) private memberRepo: Repository<Member>,
    @InjectRepository(Mission) private missionRepo: Repository<Mission>,
    @InjectRepository(MemberMission) private memberMissionRepo: Repository<MemberMission>,
  ...
  async onModuleInit() {
    ...
    if (count === 0) {
      ...
      await this.missionRepo.save([
        { title: 'Daily Breecher', description: 'Main game apapun 3 kali hari ini', code: 'PLAY_ANY_GAME', rewardPoints: 5, targetValue: 3, icon: 'Trophy', isActive: true },
        { title: 'Bom Hunter', description: 'Main Scratch Bomb 5 kali', code: 'PLAY_SCRATCH', rewardPoints: 10, targetValue: 5, icon: 'Target', isActive: true },
        { title: 'Whale Apprentice', description: 'Capai total bet 50 poin', code: 'ACCUMULATE_BET', rewardPoints: 25, targetValue: 50, icon: 'Zap', isActive: true }
      ]);
    }
  }
  ...
  async getPointLedger(memberId: number) {
    return this.ledgerRepo.find({ where: { memberId }, order: { createdAt: 'DESC' } });
  }

  async getPortalMember(id: number) {
  ...
  async redeem(memberId: number, rewardId: number) {
    ...
    try {
      const reward = await queryRunner.manager.findOne(PointReward, { where: { id: rewardId } });
      ...
      if (member.points < reward.pointCost) throw new BadRequestException('Insufficient points');
      ...
      await queryRunner.manager.save(PointLedger, ledger);
      ...
      if (reward.menuItemId) {
         this.cafeService.processOrder([
            { id: reward.menuItemId, quantity: 1, note: 'TUKAR POIN', customName: `[RWD] ${reward.name}`, priceOverride: 0 }
  ...
  async playScratchBomb(memberId: number, betAmount?: number) {
    const queryRunner = this.dataSource.createQueryRunner();
    await queryRunner.connect(); await queryRunner.startTransaction();
    ...
    try {
      ...
      const member = await queryRunner.manager.findOne(Member, { where: { id: memberId } });
      if (!member) throw new NotFoundException('Member not found');
      if (member.points < playCost) throw new BadRequestException(`Insufficient points. Need ${playCost} PTS.`);

      // Analytics: Active Players
      const fifteenMinsAgo = new Date(); fifteenMinsAgo.setMinutes(fifteenMinsAgo.getMinutes() - 15);
      const activePlayersResult = await queryRunner.manager.createQueryBuilder(PointLedger, 'ledger')
          .select('COUNT(DISTINCT ledger.memberId)', 'count').where('ledger.type = :type', { type: 'GAME_PLAY' })
          .andWhere('ledger.createdAt >= :date', { date: fifteenMinsAgo }).getRawOne();
      ...
      const pastResults = await queryRunner.manager.find(PointLedger, { 
        where: { memberId, type: In(['GAME_PLAY', 'GAME_WIN']) }, order: { createdAt: 'DESC' }, take: 15
      });

      let loseStreak = 0;
      for (const l of pastResults) { if (l.type === 'GAME_WIN') break; if (l.type === 'GAME_PLAY') loseStreak++; }

      let actualWinRate = winRate;
      let rtpModifier = "NORMAL";

       if (member.targetWinRate !== null) {
           actualWinRate = member.targetWinRate; rtpModifier = "MANUAL_OVERRIDE";
       } else {
           const totalPlays = await queryRunner.manager.count(PointLedger, { where: { memberId, type: 'GAME_PLAY' } });
           if (totalPlays < 3) { 
               actualWinRate = 80; rtpModifier = "HOOK_ACTIVE"; 
           } else if (activePlayerCount < 3) { 
               // QUIET PERIOD: High frequency win but small rewards to keep players
               actualWinRate = 45; rtpModifier = "LOW_VOLUME_BOOST"; 
           } else if (activePlayerCount >= 10) {
               // BUSY PERIOD: Higher Jackpot potential to create "Global Hype"
               actualWinRate = 18; rtpModifier = "HIGH_VOLUME_FRENZY";
           } else if (loseStreak >= 10) { 
               actualWinRate = 70; rtpModifier = "MERCY_ACTIVE"; 
       ...
       const baseRewardsList = settings.scratchBombRewards 
          ? settings.scratchBombRewards.split(',').map(Number).filter(n => !isNaN(n))
          : [1, 2, 5, 10, 20, 50, 100];
       ...
       const seed = Array.from(entropySource).reduce((a, b) => { a = ((a << 5) - a) + b.charCodeAt(0); return a & a; }, 0);
       ...
       if (isWinner) {
           ...
           if (affordableRewards.length === 0) affordableRewards = [Math.min(...rewardsList)];
           ...
           if (affordableRewards.length > 0) {
               ...
               if (Math.random() < highTierBias) {
                  ...
                  for(let i=0; i<affordableRewards.length; i++) {
                      if (r < weights[i]) { baseValue = affordableRewards[i]; break; }
                      r -= weights[i];
                  }
               }
               ...
               if (Math.random() < 0.15) {
                   ...
                   let mult = 2;
                   if (mRoll > 0.95) mult = 5; else if (mRoll > 0.85) mult = 4; else if (mRoll > 0.65) mult = 3;
                   if (baseValue * mult <= settings.scratchBombPool) { finalWin = baseValue * mult; activeMultiplier = mult; }
               }
      ...
      if (!isWinner) {
          ...
          const baitValues = [...rewardsList].sort(() => Math.random() - 0.5).slice(0, baitCount);
      ...
      await queryRunner.manager.save(PointLedger, playLedger);
      ...
      this.eventsGateway.loyaltyUpdated({ type: 'GAME_RESULT', memberId, isWin: isWinner, winReward });
  ...
  async claimScratchWin(memberId: number, playRef: string, securityHash?: string) {
    const queryRunner = this.dataSource.createQueryRunner();
    await queryRunner.connect(); await queryRunner.startTransaction();
    try {
        const ledger = await queryRunner.manager.findOne(PointLedger, { where: { memberId, referenceId: playRef, type: 'GAME_PLAY' } });
        if (!ledger) throw new NotFoundException('Session lost');
        if (ledger.description.includes('| CLAIMED')) throw new BadRequestException('Already claimed');

        const winMatch = ledger.description.match(/\| WIN:(\d+)/);
        if (!winMatch) throw new BadRequestException('Not a winner');
        ...
        if (securityHash) {
            ...
            if (expectedHash !== securityHash) {
                throw new BadRequestException('SECURITY BREACH: Invalid Payload Hash');
            }
        }

        const member = await queryRunner.manager.findOne(Member, { where: { id: memberId }, lock: { mode: 'pessimistic_write' } });
        if (!member) throw new NotFoundException('Member lost');
        
        member.points = Number(member.points) + winAmount;
        await queryRunner.manager.save(member);

        await queryRunner.manager.save(PointLedger, { 
            memberId, type: 'GAME_WIN', amount: winAmount, description: 'Claim Scratch Bomb Win', referenceId: playRef 
        });
        ...
        await queryRunner.commitTransaction();
        this.eventsGateway.loyaltyUpdated({ type: 'ADJUST', memberId, newBalance: member.points });
        return { success: true, newBalance: member.points };
    } catch (e) { await queryRunner.rollbackTransaction(); throw e; }
    finally { await queryRunner.release(); }
  }

  async buyScatter(memberId: number, tier: number, betAmount?: number) {
    const queryRunner = this.dataSource.createQueryRunner();
    await queryRunner.connect(); await queryRunner.startTransaction();
    try {
        const cost = (betAmount || 2) * Math.pow(2, tier - 1);
        const member = await queryRunner.manager.findOne(Member, { where: { id: memberId } });
        if (!member || member.points < cost) throw new BadRequestException('Insufficient points');
        member.points -= cost;
        await queryRunner.manager.save(member);
        await queryRunner.manager.save(PointLedger, { memberId, type: 'GAME_PLAY', amount: -cost, description: `Buy Scatter Tier ${tier}`, referenceId: `SC-${Date.now()}` });
        await queryRunner.commitTransaction();
        return { success: true, newBalance: member.points, extraClicks: tier === 4 ? 16 : tier === 3 ? 8 : tier === 2 ? 4 : 2 };
    } catch (e) { await queryRunner.rollbackTransaction(); throw e; }
    finally { await queryRunner.release(); }
  }

  async getMemberMissions(memberId: number) {
    const missions = await this.missionRepo.find({ where: { isActive: true } });
    const memberMissions = await this.memberMissionRepo.find({ where: { memberId } });
  ...
  async claimMissionReward(memberId: number, missionId: number) {
    const um = await this.memberMissionRepo.findOne({ where: { memberId, missionId } });
    if (!um) throw new NotFoundException('Mission record not found');
    if (!um.isCompleted) throw new BadRequestException('Mission not completed yet');
    if (um.isClaimed) throw new BadRequestException('Reward already claimed');

    const m = await this.missionRepo.findOne({ where: { id: missionId } });
    if (!m) throw new NotFoundException('Mission definition not found');
    ...
    try {
      ...
      await queryRunner.manager.save(PointLedger, {
        memberId, type: 'MISSION_REWARD', amount: m.rewardPoints,
        description: `Hadiah Misi: ${m.title}`, referenceId: `MSN-${m.id}-${Date.now()}`
      });
  ...
  async playMahjongSlot(memberId: number) {
    const queryRunner = this.dataSource.createQueryRunner();
    await queryRunner.connect(); await queryRunner.startTransaction();
    ...
    try {
      ...
      const SYMBOL_VALUES: Record<number, number> = { 1: 50, 2: 30, 3: 20, 4: 10, 5: 5, 6: 3, 7: 2, 8: 1, 9: 0.5 };
      const cascades: any[] = []; let totalWin = 0; let multiplierSteps = [1, 2, 3, 5];
      
      let currentGrid = this.generateRandomGrid(); 
      const solveCascade = (grid: number[][], stepIdx: number) => {
          ...
          cascades.push({ grid: grid.map(r => [...r]), win: stepWin, lines: winningLines, multiplier: mult, pos: winningPos });
          totalWin += stepWin;
      ...
      let attempts = 0;
      while (attempts < 15) {
          cascades.length = 0; totalWin = 0; solveCascade(currentGrid, 0);
          if (targetWin && totalWin > 0) break; if (!targetWin && totalWin === 0) break;
          currentGrid = this.generateRandomGrid(); attempts++;
      }
      ...
      await queryRunner.manager.save(PointLedger, { 
          memberId, type: 'GAME_PLAY', amount: totalWin - cost, description: `Main Mahjong Ways | Win:${totalWin}`, referenceId: `MJ-${Date.now()}` 
      });

      await queryRunner.commitTransaction();
      this.eventsGateway.loyaltyUpdated({ type: 'ADJUST', memberId, newBalance: member.points });
      this.eventsGateway.loyaltyUpdated({ type: 'SETTINGS_UPDATE', settings: { mahjongSlotPool: settings.mahjongSlotPool } });

      return { success: true, cascades, totalWin, newBalance: member.points };
    } catch (e) { await queryRunner.rollbackTransaction(); throw e; }
    finally { await queryRunner.release(); }
  }
  ...
  private checkMahjongWins(grid: number[][], values: Record<number, number>) {
      let winAmount = 0; const winningLines: any[] = []; const winningPos: [number, number][] = [];
      for (let s = 1; s <= 9; s++) {
          const counts: number[] = grid.map(reel => reel.filter(v => v === s).length);
          let matchLength = 0; for (let i = 0; i < 5; i++) { if (counts[i] > 0) matchLength++; else break; }
          if (matchLength >= 3) {
              let ways = 1; for (let i = 0; i < matchLength; i++) ways *= counts[i];
              const lineWin = ways * (values[s] || 0);
              winAmount += lineWin; winningLines.push({ symbol: s, ways, length: matchLength, win: lineWin });
              for(let r=0; r<matchLength; r++) { grid[r].forEach((v, c) => { if (v === s) winningPos.push([r, c]); }); }
          }
      }
      return { winAmount, winningLines, winningPos };
  }

  private refillGrid(grid: number[][], winPos: [number, number][]) {
  ...
  async getAllRewardsAdmin() { return this.rewardRepo.find(); }

  async createReward(data: Partial<PointReward>) { return this.rewardRepo.save(data); }

  async updateReward(id: number, data: Partial<PointReward>) {
    await this.rewardRepo.update(id, data);
    return this.rewardRepo.findOne({ where: { id } });
  }

  async deleteReward(id: number) { return this.rewardRepo.delete(id); }
  ...
  async autonomousRevenueManagementEngine() {
      ...
      this.eventsGateway.loyaltyUpdated({ type: 'SETTINGS_UPDATE', settings: { scratchBombWinRate: targetWinRate } });
  }

  async adjustPoint(memberId: number, amount: number, description: string) {
    const queryRunner = this.dataSource.createQueryRunner();
    await queryRunner.connect(); await queryRunner.startTransaction();
    try {
      const member = await queryRunner.manager.findOne(Member, { where: { id: memberId } });
      ...
      await queryRunner.manager.save(member);
      await queryRunner.manager.save(PointLedger, { memberId, type: 'ADJUSTMENT', amount: Number(amount), description, referenceId: `ADJ-${Date.now()}` });
      await queryRunner.commitTransaction();
      this.eventsGateway.loyaltyUpdated({ type: 'ADJUST', memberId, newBalance: member.points });
      return { success: true, newBalance: member.points };
    } catch (e) { await queryRunner.rollbackTransaction(); throw e; }
   ...
   async getMemberWinStats() {
       ...
       const stats = await this.ledgerRepo.createQueryBuilder('l')
           ...
           .addSelect("SUM(CASE WHEN l.type = 'GAME_PLAY' THEN ABS(l.amount) ELSE 0 END)", "ptsIn")
           .addSelect("SUM(CASE WHEN l.type = 'GAME_WIN' THEN ABS(l.amount) ELSE 0 END)", "ptsOut")
           .addSelect("COUNT(CASE WHEN l.type = 'GAME_PLAY' THEN 1 END)", "plays")
           .where("l.type IN ('GAME_PLAY', 'GAME_WIN')")
           .groupBy('l.memberId')
           .getRawMany();
       ...
       return members.map(m => {
           const s = stats.find(x => x.memberId === m.id) || { ptsIn: 0, ptsOut: 0, plays: 0 };
  ...
  async setTargetWinRate(memberId: number, targetWinRate: number | null) {
  ...
  async getGameAnalytics() {
    try {
      ...
      const ptsInRes = await this.ledgerRepo.createQueryBuilder('l')
          .select("COALESCE(SUM(ABS(l.amount)), 0)", "total")
          .where("l.type = :type", { type: 'GAME_PLAY' })
          .getRawOne();
      
      const ptsOutRes = await this.ledgerRepo.createQueryBuilder('l')
          .select("COALESCE(SUM(ABS(l.amount)), 0)", "total")
          .where("l.type = :type", { type: 'GAME_WIN' })
          .getRawOne();
      ...
      // 2. Realtime Volume
      const totalPlays = await this.ledgerRepo.count({ where: { type: 'GAME_PLAY' } });
      const fifteenMinsAgo = new Date(); fifteenMinsAgo.setMinutes(fifteenMinsAgo.getMinutes() - 15);
      const activePlayersResult = await this.ledgerRepo.createQueryBuilder('l')
          .select('COALESCE(COUNT(DISTINCT l.memberId), 0)', 'count')
          .where('l.type = :type', { type: 'GAME_PLAY' })
          .andWhere('l.createdAt >= :date', { date: fifteenMinsAgo })
          .getRawOne();
      const activePlayerCount = parseInt(activePlayersResult?.count || activePlayersResult?.COUNT || 0, 10) || 0;

      // 3. Trend Data (7 Days)
      const sevenDaysAgo = new Date(); sevenDaysAgo.setDate(sevenDaysAgo.getDate() - 7);
      const dailyData = await this.ledgerRepo.createQueryBuilder('l')
          .select("DATE(l.createdAt)", "date")
          .addSelect("COALESCE(SUM(CASE WHEN l.type = 'GAME_PLAY' THEN ABS(l.amount) ELSE 0 END), 0)", "in_val")
          .addSelect("COALESCE(SUM(CASE WHEN l.type = 'GAME_WIN' THEN ABS(l.amount) ELSE 0 END), 0)", "out_val")
          .where("l.createdAt >= :date", { date: sevenDaysAgo })
          .andWhere("l.type IN ('GAME_PLAY', 'GAME_WIN')")
          .groupBy("DATE(l.createdAt)")
          .orderBy("DATE(l.createdAt)", "ASC")
          .getRawMany();
      ...
      const trendData = dailyData.map(d => {
          ...
          cumulative += (dIn - dOut);
          return { date: d.date, amount: cumulative };
      });

      // 4. Feed & Distribution
      const recentPlays = await this.ledgerRepo.find({
          where: { type: In(['GAME_PLAY', 'GAME_WIN', 'MISSION_REWARD']) },
          relations: ['member'],
      ...
      return {
        ptsIn, ptsOut, netProfitPoints, netProfitIDR, pointValue: Number(pointValue), performance,
        totalPlays, activePlayerCount,
        pointsIn: ptsIn,
        pointsOut: ptsOut,
        ...
        mahjongStats: { rtp: 88, pool: Number(settings.mahjongSlotPool) || 0 },
        scratchStats: { rtp: 92, pool: Number(settings.scratchBombPool) || 0 }
      };
    } catch (error) {
       this.logger.error('Analytics Fetch Error', error);
       return { 
         ptsIn: 0, ptsOut: 0, netProfitPoints: 0, netProfitIDR: 0, performance: 0, 
         totalPlays: 0, activePlayerCount: 0, winPool: 0, distribution: {},
         pointsIn: 0, pointsOut: 0, netProfit: 0, activePlayers: 0,
         recentPlays: [], trendData: [], armeStatus: { systemIntegrity: 'OFFLINE' }
       };
    }
  }
  ...
  async activateEmergencyBrake() {
      ...
      await this.dataSource.getRepository(Setting).save(settings);
      this.eventsGateway.loyaltyUpdated({ type: 'SETTINGS_UPDATE', settings: { scratchBombWinRate: 0, mahjongSlotWinRate: 0, isEmergencyMode: true } });
      return { success: true, message: 'All Games Frozen' };
  }

  private async updateMissionProgress(memberId: number, code: string, increment: number) {
    try {
      const m = await this.missionRepo.findOne({ where: { code, isActive: true } });
      if (!m) return;
      let um = await this.memberMissionRepo.findOne({ where: { memberId, missionId: m.id } });
      if (!um) { um = new MemberMission(); um.memberId = memberId; um.missionId = m.id; um.currentValue = 0; }
      if (um.isCompleted) return;
      um.currentValue += increment;
      if (um.currentValue >= m.targetValue) { um.currentValue = m.targetValue; um.isCompleted = true; }
      await this.memberMissionRepo.save(um);
    } catch (e) { this.logger.error('Mission Error', e); }
  }
}
