...
export class LoyaltyService implements OnModuleInit {
  ...
  constructor(
    ...
    @InjectRepository(MemberMission)
    private memberMissionRepo: Repository<MemberMission>,
  ...
  async getPortalMember(id: number) {
    ...
    return {
      ...
      winPool:
        (Number(settings.scratchBombPool) || 0) +
        (Number(settings.mahjongSlotPool) || 0),
    };
  }
  ...
  async redeem(memberId: number, rewardId: number) {
    ...
    try {
      const reward = await queryRunner.manager.findOne(PointReward, {
        where: { id: rewardId },
      });
      ...
      member.points = Math.max(
        0,
        Number(member.points) - Number(reward.pointCost),
      );
  ...
  async confirmRedeem(token: string) {
    ...
    try {
      ...
      if (playCost > 0 && member.points < playCost) {
        throw new BadRequestException(
          `Insufficient points. Need ${playCost} PTS.`,
        );
      }
      ...
      const activePlayersResult = await queryRunner.manager
        .createQueryBuilder(PointLedger, 'ledger')
        .select('COUNT(DISTINCT ledger.memberId)', 'count')
        .where('ledger.type = :type', { type: 'GAME_PLAY' })
        .andWhere('ledger.createdAt >= :date', { date: fifteenMinsAgo })
        .getRawOne();
      ...
      const pastResults = await queryRunner.manager.find(PointLedger, {
        where: { memberId, type: In(['GAME_PLAY', 'GAME_WIN']) },
        order: { createdAt: 'DESC' },
        take: 15,
      });
      ...
      if (member.targetWinRate !== null) {
        ...
        const totalPlays = await queryRunner.manager.count(PointLedger, {
          where: { memberId, type: 'GAME_PLAY' },
        });
        if (totalPlays < 3) {
          actualWinRate = 80;
      ...
      if (isWinner) {
        let affordableRewards = rewardsList.filter(
          (r) => r <= settings.scratchBombPool,
        );
        if (affordableRewards.length === 0)
          affordableRewards = [Math.min(...rewardsList)];
        ...
        if (affordableRewards.length > 0) {
          ...
          if (Math.random() < highTierBias) {
            ...
            baseValue =
              affordableRewards[
                Math.floor(affordableRewards.length / 2) +
                  Math.floor(Math.random() * (affordableRewards.length / 2))
              ];
          } else {
            const weights = affordableRewards.map(
              (_, i) => affordableRewards.length - i,
            );
      ...
      playLedger.description = isWinner
        ? `Main Scratch Bomb | WIN:${winReward} | SYM:${winningSymbol}`
        : 'Main Scratch Bomb';
      ...
      const payloadHash = crypto
        .createHash('sha256')
        .update(`${memberId}-${playLedger.referenceId}-${winReward}`)
        .digest('hex');
  ...
  async claimScratchWin(
    ...
    await queryRunner.startTransaction();
    try {
      const ledger = await queryRunner.manager.findOne(PointLedger, {
        where: { memberId, referenceId: playRef, type: 'GAME_PLAY' },
      });
      if (!ledger) throw new NotFoundException('Session lost');
      if (ledger.description.includes('| CLAIMED'))
      ...
      if (securityHash) {
        ...
        const expectedHash = crypto
          .createHash('sha256')
          .update(`${memberId}-${playRef}-${winAmount}`)
          .digest('hex');
        if (expectedHash !== securityHash) {
          throw new BadRequestException(
            'SECURITY BREACH: Invalid Payload Hash',
          );
        }
      }
  ...
  async playMahjongSlot(memberId: number) {
    ...
    try {
      ...
      if (member.targetWinRate !== null)
        targetWin = Math.random() * 100 < member.targetWinRate;
      ...
      const cascades: any[] = [];
      let totalWin = 0;
      const multiplierSteps = [1, 2, 3, 5];

      let currentGrid = this.generateRandomGrid();
      const solveCascade = (grid: number[][], stepIdx: number) => {
        const { winAmount, winningLines, winningPos } = this.checkMahjongWins(
          grid,
          SYMBOL_VALUES,
        );
        ...
        if (stepWin > 0 && totalWin + stepWin > settings.mahjongSlotPool * 0.4)
          return;
        ...
        totalWin += stepWin;
        if (stepWin > 0)
          solveCascade(this.refillGrid(grid, winningPos), stepIdx + 1);
      };
      ...
      this.eventsGateway.loyaltyUpdated({
        type: 'SETTINGS_UPDATE',
        settings: { mahjongSlotPool: settings.mahjongSlotPool },
      });
  ...
  private checkMahjongWins(grid: number[][], values: Record<number, number>) {
    ...
    const winningPos: [number, number][] = [];
    for (let s = 1; s <= 9; s++) {
      const counts: number[] = grid.map(
        (reel) => reel.filter((v) => v === s).length,
      );
      let matchLength = 0;
      for (let i = 0; i < 5; i++) {
  ...
  async createReward(data: Partial<PointReward>) {
  ...
  async getMemberWinStats() {
    ...
    const stats = await this.ledgerRepo
      ...
      .addSelect(
        "SUM(CASE WHEN l.type = 'GAME_PLAY' THEN ABS(l.amount) ELSE 0 END)",
        'ptsIn',
      )
      .addSelect(
        "SUM(CASE WHEN l.type = 'GAME_WIN' THEN ABS(l.amount) ELSE 0 END)",
        'ptsOut',
      )
    ...
    return members.map((m) => {
      ...
      return {
        ...
        actualWinRate:
          inPts > 0 ? Number(((outPts / inPts) * 100).toFixed(1)) : 0,
  ...
  async getGameAnalytics() {
    try {
      ...
      const pointValue =
        parseFloat(settings.royaltyPointsPerAmount?.toString() || '1000') ||
        1000;
      const targetSurplus =
        parseFloat(
          settings.gamificationTargetSurplus?.toString() || '5000000',
        ) || 5000000;

      // 1. Core Totals - Use COALESCE for strict 0 values from SQL
      const ptsInRes = await this.ledgerRepo
        .createQueryBuilder('l')
        .select('COALESCE(SUM(ABS(l.amount)), 0)', 'total')
        .where('l.type = :type', { type: 'GAME_PLAY' })
        .getRawOne();

      const ptsOutRes = await this.ledgerRepo
        .createQueryBuilder('l')
        .select('COALESCE(SUM(ABS(l.amount)), 0)', 'total')
      ...
      const totalPlays = await this.ledgerRepo.count({
        where: { type: 'GAME_PLAY' },
      });
      const fifteenMinsAgo = new Date();
      fifteenMinsAgo.setMinutes(fifteenMinsAgo.getMinutes() - 15);
      const activePlayersResult = await this.ledgerRepo
        .createQueryBuilder('l')
      ...
      const activePlayerCount =
        parseInt(
          activePlayersResult?.count || activePlayersResult?.COUNT || 0,
          10,
        ) || 0;
      ...
      const dailyData = await this.ledgerRepo
        .createQueryBuilder('l')
        .select('DATE(l.createdAt)', 'date')
        .addSelect(
          "COALESCE(SUM(CASE WHEN l.type = 'GAME_PLAY' THEN ABS(l.amount) ELSE 0 END), 0)",
          'in_val',
        )
        .addSelect(
          "COALESCE(SUM(CASE WHEN l.type = 'GAME_WIN' THEN ABS(l.amount) ELSE 0 END), 0)",
          'out_val',
        )
      ...
      return {
        ...
        activePlayers: activePlayerCount,
        currentStrategy: settings.isEmergencyMode
          ? 'EMERGENCY_BRAKE'
          : ptsIn === 0
            ? 'IDLE'
            : 'STABLE',
        ...
        distribution,
        armeStatus: {
          systemIntegrity: settings.isEmergencyMode
            ? 'FROZEN'
            : ptsIn === 0
              ? 'IDLE'
              : 'STABLE',
      ...
      return {
        ...
        recentPlays: [],
        trendData: [],
        armeStatus: { systemIntegrity: 'OFFLINE' },
      };
    }
  }
  ...
  private async updateMissionProgress(
    ...
    try {
      ...
      let um = await this.memberMissionRepo.findOne({
        where: { memberId, missionId: m.id },
      });
      if (!um) {
        um = new MemberMission();
