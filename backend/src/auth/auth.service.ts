...
export class AuthService {
    ...
    async createAccessRequest(user: any, socketId?: string) {
        ...
        const request = this.accessRequestRepository.create({
            ...
            roleName: (user.role?.name?.toUpperCase() || user.role?.toUpperCase() || 'USER') as string,
            isOutOfShift,
    ...
    async approveAccessRequest(requestId: number, adminId: number, adminName: string, note?: string) {
        ...
        if (this.eventsGateway.server) {
            this.eventsGateway.server.emit('access_request_handled', { requestId: saved.id });
        }

        return saved;
    }

    async denyAccessRequest(requestId: number, adminId: number, adminName: string, note?: string) {
        const request = await this.accessRequestRepository.findOne({ where: { id: requestId } });
        if (!request) throw new BadRequestException('Request not found');
