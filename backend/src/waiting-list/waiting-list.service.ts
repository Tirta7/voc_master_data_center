...
import { WaitingList, WaitingListStatus, WaitingListType } from './entities/waiting-list.entity';
...
export class WaitingListService {
    ...
    async assignToTable(id: number, tableId: number, userId: number, userName: string) {
        ...
        if (entry.handledById && entry.handledById !== userId) {
            throw new BadRequestException(`Antrean ini sedang dihandle oleh ${entry.handledByName}.`);
        }
        ...
        if (entry.targetTableId && entry.targetTableId !== tableId) {
            if (entry.type === WaitingListType.CAFE) {
                const oldTable = await this.cafeTableRepository.findOne({ where: { id: entry.targetTableId } });
                if (oldTable && oldTable.bookedByWaitingId === entry.id) {
                    ...
                    this.billiardGateway.server.emit('tableUpdate', { ...oldTable, type: 'cafe' });
                }
            } else {
                const oldTable = await this.tableRepository.findOne({ where: { id: entry.targetTableId } });
                if (oldTable && oldTable.bookedByWaitingId === entry.id) {
                    oldTable.isBooked = false;
                    oldTable.bookedByWaitingId = null as any;
                    oldTable.bookedByName = null as any;
        ...
        let tableName = '';
        if (entry.type === WaitingListType.CAFE) {
            const table = await this.cafeTableRepository.findOne({ where: { id: tableId } });
            ...
            this.billiardGateway.server.emit('tableUpdate', { ...table, type: 'cafe' });
        } else {
    ...
    async cancel(id: number, userId: number, userName: string) {
        ...
        if (entry.handledById && entry.handledById !== userId) {
            throw new ForbiddenException(`Anda tidak dapat membatalkan antrean ini karena sedang dihandle oleh ${entry.handledByName}.`);
        }
        ...
        if (entry.targetTableId) {
            if (entry.type === WaitingListType.CAFE) {
                const table = await this.cafeTableRepository.findOne({ where: { id: entry.targetTableId } });
                ...
                const table = await this.tableRepository.findOne({ where: { id: entry.targetTableId } });
                if (table && table.bookedByWaitingId === entry.id) {
                    table.isBooked = false;
                    table.bookedByWaitingId = null as any;
                    table.bookedByName = null as any;
    ...
    async unassignTable(id: number, userId: number, userName: string) {
        ...
        if (entry.handledById && entry.handledById !== userId) {
            throw new ForbiddenException(`Anda tidak dapat merubah antrean ini karena sedang dihandle oleh ${entry.handledByName}.`);
        }
    ...
    async handle(id: number, userId: number, userName: string) {
        ...
        if (entry.targetTableId) {
            throw new BadRequestException('Antrean sudah ditugaskan ke meja dan tidak dapat dikeep.');
        }

        // Check if already kept by someone else
        if (entry.handledById && entry.handledById !== userId) {
            throw new BadRequestException(`Antrean sudah dikeep oleh ${entry.handledByName}`);
        }
    ...
    async unhandle(id: number, userId: number, userName: string) {
        ...
        if (entry.handledById !== userId) {
            throw new ForbiddenException('Anda tidak memiliki akses untuk melepas antrean ini.');
        }
