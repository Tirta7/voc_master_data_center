...
export class WaitingListController {
  ...
  @Post('public')
  createPublic(@Body() data: Partial<WaitingList>) {
  ...
  assign(
    ...
    return this.waitingListService.assignToTable(
      +id,
      tableId,
      req.user.id,
      req.user.username,
  ...
  unassign(@Param('id') id: string, @Request() req: any) {
    return this.waitingListService.unassignTable(
      +id,
      req.user.id,
      req.user.username,
    );
  ...
  unhandle(@Param('id') id: string, @Request() req: any) {
    return this.waitingListService.unhandle(
      +id,
      req.user.id,
      req.user.username,
    );
