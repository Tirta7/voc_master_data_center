...
export class TransactionService {
    ...
    async getActiveTransactionByTable(tableId: number): Promise<Transaction | null> {
        ...
        if (transaction && transaction.status === TransactionStatus.PAID) {
            if (!transaction.table || transaction.table.status === TableStatus.AVAILABLE) {
                return null;
            }
        }

        if (transaction && transaction.table && transaction.table.startTime && transaction.table.status !== TableStatus.AVAILABLE) {
            ...
            if (transaction.table.sessionType === 'open') {
                ...
                if (transaction.table.packageId) {
                    pkg = await this.packageRepository.findOne({ where: { id: transaction.table.packageId } }) || {};
                } else {
     ...
     */
    private calculateVitals(transaction: Transaction, settings: any): Partial<Transaction> & { tierDiscountAmount: number } {
    ...
    calculateTimeBasedPrice(startTime: Date, endTime: Date, pkg: any): { total: number, details: any[] } {
        ...
        while (current < calculationEnd) {
            ...
            if (!currentSegment || currentSegment.title !== slotName) {
                if (currentSegment) {
                    ...
                    currentSegment.endTimeFormatted = current.toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit', hour12: false }).replace(':', '.');
                    details.push(currentSegment);
                }
                currentSegment = {
                    title: slotName,
                    startTimeFormatted: current.toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit', hour12: false }).replace(':', '.'),
                    duration: 0,
                    cost: 0,
        ...
        if (currentSegment) {
            currentSegment.subtotal = Math.round(currentSegment.cost);
            currentSegment.endTimeFormatted = calculationEnd.toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit', hour12: false }).replace(':', '.');
            details.push(currentSegment);
        }
    ...
    async processMultiPayerPayment(
        ...
        try {
            ...
            if (Number(savedTx.paidAmount) >= Number(savedTx.grandTotal) - 1) { // 1 tolerance for rounding
    ...
    async updateTotals(transactionOrId: number | Transaction): Promise<Transaction> {
        ...
        if (typeof transactionOrId === 'number') {
            ...
            foundTx = await this.transactionRepository.findOne({
                ...
                relations: ['orderItems', 'table', 'member', 'member.tier'],
            });
        ...
        let finalVitals = this.calculateVitals({ billiardTotal, orderItems } as any, settings);
