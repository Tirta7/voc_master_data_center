...
export class InvoiceService {
  ...
  async generateThermalInvoice(transaction: Transaction): Promise<string> {
      ...
      const printCategory = (cat: string) => {
        ...
        items.forEach((item) => {
          if (item.bundleGroupId) {
            ...
            const bundleItems = transaction.orderItems.filter(
              (i) => i.bundleGroupId === item.bundleGroupId,
            );
            const bundleName =
              bundleItems.find((i) => i.customName?.includes('[PAKET]'))
                ?.customName ||
              `Paket: ${item.note?.replace('Bundle: ', '') || 'Promo'}`;
            ...
            if (bundlePaid) {
              ...
              lines.push(
                `1 x ${bundleTotal.toLocaleString()} [WALLET]         Rp. 0`,
              );
            } else {
    ...
    const memberBilliardPaid = (transaction.payments || [])
      .filter(
        (p: any) =>
          p.paymentMethod === 'MEMBER' && Number(p.billiardPortion) > 0,
      )
      .reduce((sum: number, p: any) => sum + Number(p.billiardPortion), 0);
    const billiardIsPrepaidWallet =
      memberBilliardPaid >= billiardTotal && billiardTotal > 0;

    if (billiardTotal > 0) {
      if (billiardIsPrepaidWallet) {
        lines.push(
          `Total Table : Rp. ${billiardTotal.toLocaleString()} [WALLET]`,
        );
      } else {
  ...
  async generateThermalReceipt(
    ...
    if (payment.itemsSnapshot && Array.isArray(payment.itemsSnapshot)) {
      ...
      payment.itemsSnapshot.forEach((item: any) => {
        if (item.bundleGroupId) {
          ...
          const bundleItems = payment.itemsSnapshot.filter(
            (i: any) => i.bundleGroupId === item.bundleGroupId,
          );
          const bundleName =
            bundleItems.find((i: any) => i.displayName?.includes('[PAKET]'))
              ?.displayName || `Paket: Bundle`;
          const bundleTotal = bundleItems.reduce(
    ...
    if (Number(payment.billiardPortion) > 0) {
      const label = transaction.fareName
        ? `Billiard (${transaction.fareName})`
        : 'Billiard Portion';
