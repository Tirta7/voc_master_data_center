...
export class UserService {
    ...
    async createEmployee(userData: any) {
        try {
            ...
            if (existingUser) {
                if (existingUser.username === username) throw new ConflictException('Username sudah terdaftar');
                if (existingUser && email && existingUser.email === email) throw new ConflictException('Email sudah terdaftar');
            }

            // Extract User Identity fields only
            const userFields = ['name', 'placeOfBirth', 'dateOfBirth', 'gender', 'address', 'religion', 'maritalStatus', 'jobTitle', 'nationality', 'joinedAt', 'phone', 'baseShift'];
    ...
    async updateEmployee(id: number, userData: any) {
        ...
        let payroll = await this.payrollRepository.findOne({ where: { user: { id } } });
        if (payroll) {
    ...
    async deleteEmployee(id: number) {
        ...
        await Promise.all([
            ...
            this.transactionRepository.update({ createdByUserId: id }, { createdByUserId: null as any }),
            this.transactionRepository.update({ openedByUserId: id }, { openedByUserId: null as any }),
            this.orderItemRepository.update({ createdByUserId: id }, { createdByUserId: null as any }),
            this.orderItemRepository.update({ completedByUserId: id }, { completedByUserId: null as any }),
            this.statusLogRepository.delete({ user: { id } }),

            this.userRepository.manager.createQueryBuilder()
                .update('shifts')
                .set({ userId: null })
                .where('userId = :id', { id })
                .execute(),
    ...
    async findById(id: number) {
        return this.userRepository.findOne({ where: { id }, relations: ['payrollConfig'] });
    }
    ...
    async deleteRole(id: number) {
        const usersCount = await this.userRepository.count({ where: { role: { id } } });
        if (usersCount > 0) {
    ...
    async calculateMonthlyPayroll(userId: number, month: number, year: number) {
        ...
        const config = await this.payrollRepository.findOne({ where: { user: { id: userId } } });
        if (!config) return null;

        // 1. Service Commission (Table Starts)
        const tableStarts = await this.transactionRepository.count({
            where: [
                { commissionUserId: userId, createdAt: Between(startDate, endDate), status: Not(TransactionStatus.CANCELLED) },
                { commissionUserId: IsNull(), createdByUserId: userId, createdAt: Between(startDate, endDate), status: Not(TransactionStatus.CANCELLED) }
            ]
        });
        ...
        const salesItems = await this.orderItemRepository
            ...
            .where('(oi.commissionUserId = :userId OR (oi.commissionUserId IS NULL AND oi.createdByUserId = :userId))', { userId })
            .andWhere('oi.createdAt BETWEEN :start AND :end', { start: startDate, end: endDate })
            .andWhere('oi.status IN (:...statuses)', { statuses: [OrderItemStatus.DONE, OrderItemStatus.QUEUED, OrderItemStatus.PROCESSING] })
            .andWhere('(oi.isPaid = true OR t.status != :cancelledStatus)', { cancelledStatus: TransactionStatus.CANCELLED })
            .getMany();

        const categoryBreakdown: Record<string, { volume: number; commission: number; percent: number }> = {};
        ...
        Object.entries(commissionsMap).forEach(([cat, val]) => {
            ...
            if (!displayNamesMap[key]) {
                ...
                categoryBreakdown[displayNamesMap[key]] = { volume: 0, commission: 0, percent };
            }
        });
        ...
        salesItems.forEach(item => {
            ...
            const percent = normalizedCommissionsMap[categoryKey] !== undefined
        ...
        const productionItems = await this.orderItemRepository
            ...
            .andWhere('oi.completedAt BETWEEN :start AND :end', { start: startDate, end: endDate })
            .andWhere('oi.status = :status', { status: OrderItemStatus.DONE })
            .andWhere('(oi.isPaid = true OR t.status != :cancelledStatus)', { cancelledStatus: TransactionStatus.CANCELLED })
            .getMany();

        const productionBreakdown: Record<string, { volume: number; commission: number; percent: number }> = {};
        ...
        Object.entries(commissionsMap).forEach(([cat, val]) => {
            ...
            if (!prodDisplayNamesMap[key]) {
                ...
                productionBreakdown[prodDisplayNamesMap[key]] = { volume: 0, commission: 0, percent };
            }
        });
        ...
        productionItems.forEach(item => {
            ...
            if (!productionBreakdown[displayName]) {
                productionBreakdown[displayName] = { volume: 0, commission: 0, percent };
            }
        ...
        const sessions = await this.transactionRepository.count({
            where: [
                ...
                { commissionUserId: IsNull(), createdByUserId: userId, createdAt: Between(startDate, endDate) }
            ]
        });
        ...
        const activeDaysResult = await this.statusLogRepository
            ...
            .andWhere('log.startedAt BETWEEN :start AND :end', { start: startDate, end: endDate })
    ...
    async getDetailedPayrollReport(userId: number, month: number, year: number) {
        ...
        const dailySummary: Record<string, { active: number; away: number; offline: number }> = {};
        statusLogs.forEach(log => {
            const date = new Date(log.startedAt).toISOString().split('T')[0];
            if (!dailySummary[date]) dailySummary[date] = { active: 0, away: 0, offline: 0 };

            const duration = log.durationSeconds || 0;
            if (log.status === UserStatus.ACTIVE) dailySummary[date].active += duration;
            else if (log.status === UserStatus.AWAY) dailySummary[date].away += duration;
    ...
    async hasActiveShift(userId: number): Promise<boolean> {
        const count = await this.userRepository.manager.createQueryBuilder()
            .select('id')
            .from('shifts', 's')
            .where('s.userId = :userId AND s.status = :status', { userId, status: 'OPEN' })
            .getCount();
        return count > 0;
    }
}
