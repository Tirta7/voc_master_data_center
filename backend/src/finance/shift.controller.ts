import { Controller, Get, Post, Body, Param, UseGuards, Request, ParseFloatPipe, UnauthorizedException } from '@nestjs/common';
...
export class ShiftController {
    ...
    @Post(':id/assignments')
    async updateAssignments(@Param('id') id: number, @Body('assignedTableIds') assignedTableIds: any[]) {
        return this.shiftService.updateAssignments(id, assignedTableIds);
    }

    @Post('user/:userId/assignments')
    async updateUserAssignments(@Param('userId') userId: number, @Body('assignedTableIds') assignedTableIds: any[]) {
        return this.shiftService.updatePersistentAssignments(userId, assignedTableIds);
    }

    @Post('start')
    async startShift(@Request() req: any, @Body() body: { cashStart: number; shiftName?: string; assignedTableIds?: any[] }) {
    ...
    async endShift(
        ...
        if (targetUserId !== req.user.id) {
            ...
            if (!['ADMIN', 'OWNER'].includes(userRole)) {
                throw new UnauthorizedException('Hanya Admin yang dapat mengakhiri shift staf lain.');
            }
        }
