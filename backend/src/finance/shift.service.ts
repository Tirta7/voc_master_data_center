...
export class ShiftService {
  ...
  async getOrCreateActiveBusinessDay(): Promise<BusinessDay> {
    ...
    const dateString = `${logicalDate.getFullYear()}-${String(
      logicalDate.getMonth() + 1,
    ).padStart(2, '0')}-${String(logicalDate.getDate()).padStart(2, '0')}`;
    ...
    if (
      ...
      const [h, m] = (settings.autoSettlementTime || '04:00')
        .split(':')
        .map(Number);
      ...
      if (now > deadline) {
        ...
        if (activeSessions === 0) {
          ...
          this.logger.warn(
            `Business Day #${activeDay.id} is stale but has ${activeSessions} active sessions. Skipping auto-settle.`,
          );
        }
      }
    }
  ...
  async getSettlementStatus(): Promise<{
    ...
    if (!activeDay) {
      return { isStale: false, canAutoSettle: false, autoSettlementEnabled };
    }
    ...
    return {
      ...
      reason:
        activeSessions > 0
          ? `${activeSessions} active sessions present`
          : undefined,
      businessDayId: activeDay.id,
      businessDayDate: activeDay.date,
      settlementDeadline: deadline,
  ...
  async startShift(
    ...
    if (!acquired) {
      this.logger.warn(
        `startShift: Shift start for user ${userId} is already in progress.`,
      );
  ...
  async getActiveShift(userId: number): Promise<Shift | null> {
    ...
    if (shift) {
      try {
        ...
        (shift as any).paymentMethods = breakdown.paymentMethods;
      } catch (error) {
        ...
        (shift as any).paymentMethods = { CASH: 0, QRIS: 0, TRANSFER: 0, MEMBER: 0 };
      }
  ...
  async updateActiveShift(
    userId: number,
    data: { cashStart?: number; shiftName?: string },
  ): Promise<Shift> {
   ...
   */
  async calculateExpectedCash(shiftId: number): Promise<{
    ...
    const ledgerEntries = await this.cashflowRepo.find({
      where: [
        { shiftId, source: In(['manual', 'stock_purchase']) },
        {
          businessDayId: shift.businessDayId,
          source: In(['manual', 'stock_purchase']),
          timestamp: Between(shift.startTime, shift.endTime || new Date()),
        },
      ],
    });
   ...
   */
  public getDepartmentsByRole(roleName: string): string[] {
    const role = (roleName || '').toUpperCase();
    if (role.includes('KITCHEN') || role.includes('DAPUR')) return ['KITCHEN'];
    ...
    if (
      ...
      return ['KITCHEN', 'BAR', 'CASHIER'];
    }
    if (role.includes('WAITER') || role.includes('PELAYAN')) return ['WAITER'];
    return [];
  }
  ...
  async endShift(
    ...
    if (!acquired) {
      this.logger.warn(
        `endShift: Shift end for user ${userId} is already in progress.`,
      );
    ...
    try {
      ...
      if (userDepts.length > 0) {
        ...
        for (const dept of userDepts) {
          ...
          const isProvidingReportNow = stockReports && stockReports.length > 0 && (
            (dept === 'CASHIER' && (userRole.includes('CASHIER') || userRole.includes('KASIR') || userRole.includes('ADMIN') || userRole.includes('OWNER'))) ||
            (dept === 'KITCHEN' && userRole.includes('KITCHEN')) ||
            (dept === 'BAR' && userRole.includes('BAR'))
          );
      ...
      if (closingConfig.length > 0 && requiresApproval) {
        ...
        const existingReports = await this.shiftStockReportRepo.find({ where: { shiftId: shift.id } });
        const auditSummary: any[] = existingReports.map(r => ({
          name: r.itemName,
        ...
        if (stockReports && stockReports.length > 0) {
          ...
          if (reportingDept) {
            ...
            for (const r of stockReports) {
              ...
              if (r.ingredientId) {
                const ing = await this.ingredientRepo.findOneBy({ id: r.ingredientId });
                if (ing) { sysStock = Number(ing.stockQuantity); itemName = ing.name; }
              } else if (r.menuItemId) {
                const menu = await this.menuItemRepo.findOneBy({ id: r.menuItemId });
                if (menu) { sysStock = Number((menu as any).stockQuantity || 0); itemName = menu.name; }
              }
  ...
  async notifyOwnerShiftClosed(shiftId: number) {
    try {
      const shift = await this.shiftRepo.findOne({
        where: { id: shiftId },
        relations: ['user'],
  ...
  async calculateShiftPerformance(shiftId: number): Promise<any> {
    ...
    transactions.forEach((tx) => {
      ...
      if (tx.type === TransactionType.BILLIARD) {
        if (tx.billingDetails && Array.isArray(tx.billingDetails)) {
          tx.billingDetails.forEach((detail: any) => {
      ...
      for (const report of reports) {
        ...
        if (report.ingredientId) {
          ...
          if (ing) {
            ...
            await queryRunner.manager.save(Ingredient, ing);
          }
        } else if (report.menuItemId) {
          ...
          if (menu) {
            ...
            // PHASE 6: Sync physical stock back to database
            (menu as any).stockQuantity = Number(report.physicalStock);
            await queryRunner.manager.save(MenuItem, menu);
          }
        }
   ...
   */
  async getPendingStockItems(shiftId: number, department: string) {
    const shift = await this.shiftRepo.findOne({ where: { id: shiftId } });
    if (!shift) throw new NotFoundException('Shift tidak ditemukan.');

    const ingredients = await this.ingredientRepo.find({
      where: department === 'ALL' ? {} : { department: Raw(alias => `UPPER(${alias}) = :dept`, { dept: department.toUpperCase() }) },
    });
    ...
    const checkStatus = async (items: any[], type: 'INGREDIENT' | 'MENU_ITEM') => {
      const results = [];
      const now = new Date();
      ...
      for (const item of items) {
        ...
        let reportedStatus: 'PENDING' | 'DONE' = 'PENDING';
        ...
        results.push({ ...item, reportedStatus });
      }
      return results;
    };
    ...
    const menuItemsWithCalculatedStock = menuItemsWithStatus.map((item: any) => {
    ...
    const finalMenuItems = menuItemsWithCalculatedStock.filter(m => {
      return !ingredientsWithStatus.some(ing => 
        ing.name.toLowerCase() === m.name.toLowerCase()
      );
    });
        ...
        if (tx.status === 'PAID' || Number(tx.paidAmount) >= Number(tx.grandTotal)) {
          ...
          if (txCafe === 0 && tx.orderItems && tx.orderItems.length > 0) {
            tx.orderItems.forEach((oi: any) => {
              if (
                oi.status?.toUpperCase() !== 'CANCELLED' &&
                oi.status?.toUpperCase() !== 'CANCEL_REQUESTED'
              ) {
                txCafe +=
                  Number(oi.price || oi.priceAtOrder || 0) *
          ...
          if (tx.status === 'PAID' || Number(tx.paidAmount) >= Number(tx.grandTotal)) {
            ...
            if (tx.tableId && tx.table) {
              ...
              if (!sTablePerformance[tName]) {
                sTablePerformance[tName] = { name: tName, sessions: 0, revenue: 0 };
              }
  ...
  async closeBusinessDay(id: number): Promise<BusinessDay> {
    ...
    const activeSessions = await this.sessionRepo.count({
      where: { isPaid: false, endTime: IsNull() },
    });

    if (activeSessions > 0) {
      throw new ConflictException(
        `Gagal tutup buku: Masih ada ${activeSessions} sesi billiard yang sedang berjalan / belum dibayar.`,
      );
    }
    ...
    if (openTransactions > 0) {
      throw new ConflictException(
        `Gagal tutup buku: Masih ada ${openTransactions} transaksi (order) yang belum dibayar / diselesaikan.`,
      );
    }
    ...
    businessDay.totalRevenue = transactions
      .filter(
        (t) =>
          t.status === TransactionStatus.PAID ||
          t.status === TransactionStatus.DEBT ||
    ...
    businessDay.totalExpenses = expenses.reduce(
  ...
  async getActiveAdmin(): Promise<{ id: number; name: string } | null> {
    ...
    const adminWithShift = openShifts.find((s) =>
      ['ADMIN', 'OWNER', 'CASHIER', 'SUPERADMIN', 'SUPER ADMIN', 'MANAGER'].includes(s.user?.role?.name?.toUpperCase()),
    );
