...
// import * as brain from 'brain.js'; // Removed to fix gl binding issue
...
export class AIService {
  ...
  constructor(
    ...
    private battlePlanRepo: Repository<BattlePlan>,
    @InjectRepository(BattlePlanItem)
    private battlePlanItemRepo: Repository<BattlePlanItem>,
    @InjectRepository(MenuItem)
    private menuItemRepo: Repository<MenuItem>,
    @InjectRepository(BilliardPackage)
    private billiardPackageRepo: Repository<BilliardPackage>,
    @InjectRepository(Transaction)
    private transactionRepo: Repository<Transaction>,
    @InjectRepository(BusinessDay)
    private businessDayRepo: Repository<BusinessDay>,
    @InjectRepository(OrderItem)
    private orderItemRepo: Repository<OrderItem>,
    @InjectRepository(User)
    private userRepo: Repository<User>,
    @InjectRepository(UpsellPrompt)
    private upsellPromptRepo: Repository<UpsellPrompt>,
  ...
  async onModuleInit() {
    ...
    if (!fs.existsSync(this.AI_STORAGE_DIR)) {
      fs.mkdirSync(this.AI_STORAGE_DIR, { recursive: true });
    }
  ...
  async calculateTargetMix(targetRevenue: number): Promise<any> {
    ...
    const runSolver = (isFallback: boolean = false) => {
      const model: any = {
      ...
      const lpSolver = (solver as any).Solve ? solver : (solver as any).default;
  ...
  async suggestDailyTarget(): Promise<{
    ...
    const rawDow = await this.businessDayRepo
      .createQueryBuilder('bd')
      .select('EXTRACT(DOW FROM bd.date)', 'dow')
      .addSelect('AVG(CAST(bd.totalRevenue AS FLOAT))', 'avgRevenue')
      .addSelect('COUNT(bd.id)', 'dayCount')
      .where('bd.isClosed = :closed', { closed: true })
      .groupBy('EXTRACT(DOW FROM bd.date)')
      .getRawMany();

    const dowMap: Record<number, { avg: number; count: number }> = {};
    rawDow.forEach((r) => {
    ...
    const overallAvgRevenue = history30.length > 0
      ? history30.reduce((s, d) => s + Number(d.revenue), 0) / history30.length
      : 0;

    // === 3. Recent 7-day average ===
    const recent7 = history30.slice(-7);
    const recent7Avg = recent7.length > 0
      ? recent7.reduce((s, d) => s + Number(d.revenue), 0) / recent7.length
      : 0;
    ...
    const suggestedTarget = Math.max(
      Math.round(finalTargetRaw / 25000) * 25000,
      100000 // Minimum floor
    );
   ...
   */
  async autoSuggestAndPublish(): Promise<{
    target: number;
    justification: string;
    confidence: number;
    breakdown: any;
    ...
    const strategyBrief =
      `[AI AUTO-GENERATED] Target: Rp ${target.toLocaleString('id-ID')}. ` +
    ...
    const plan = await this.createOrUpdateBattlePlan({
      ...
      items: simulation.items.map((r: any) => ({
        id: r.id,
        type: r.type,
   ...
   */
  async classifyMenuItems(): Promise<Record<number, string>> {
  ...
  private async fetchBilliardSalesHistory(days: number): Promise<any[]> {
    ...
    const data = await this.transactionRepo
      ...
      .select('t.packageId', 'packageId')
      .addSelect('COUNT(t.id)', 'totalSold')
      .where('t.status = :status', { status: TransactionStatus.PAID })
      .andWhere('t.createdAt >= :cutoff', { cutoff })
      .andWhere('t.packageId IS NOT NULL')
      .groupBy('t.packageId')
      .getRawMany();

    this.billiardHistoryCache = { data, timestamp: now };
    return data;
  }
   ...
   */
  async calculatePerformanceAchievement(businessDayId: number) {
    try {
  ...
  private async runLSTMPrediction(data: any[]): Promise<any> {
    ...
    for (let i = 0; i < normRev.length - windowSize; i++) {
      ...
      for (let j = 0; j < windowSize; j++) {
        window.push([normRev[i + j], normCount[i + j]]);
      }
      xsArr.push(window);
      ysArr.push([normRev[i + windowSize], normCount[i + windowSize]]);
    }
    ...
    // Predict next
    const lastWindow = [xsArr[xsArr.length - 1]];
  ...
  private async calculatePeakHoursFromData(days: number): Promise<string[]> {
    const cutoff = new Date();
    cutoff.setDate(cutoff.getDate() - days);
  ...
  async generateDailyStrategyBrief() {
    ...
    try {
      ...
      strategyBrief += `Prioritaskan ${strategyMatch.items.filter((i: any) => i.type === 'BILLIARD').length} Paket Billiard untuk filling occupancy.`;

      await this.createOrUpdateBattlePlan({
        businessDayId: activeBday.id,
        targetRevenue: prediction.predictedRevenue,
        items: strategyMatch.items.map((r: any) => ({
          id: r.id,
          type: r.type,
          targetQuantity: r.targetQuantity,
  ...
  async createOrUpdateBattlePlan(data: any): Promise<BattlePlan> {
    let plan = await this.battlePlanRepo.findOne({
      where: { businessDayId: data.businessDayId },
    });
  ...
  async reoptimizeBattlePlan(businessDayId: number): Promise<BattlePlan> {
  ...
  async broadcastUpsellPrompt(tableId: number, tableName: string) {
    ...
    const promptRecord = new UpsellPrompt();
  ...
  async getDailyMissionReport(businessDayId: number) {
    ...
    let totalRoiValue = 0;
    Object.values(campaignStats).forEach((stat: any) => {
  ...
  async getWaiterPerformance(businessDayId?: number) {
    ...
    const items = await this.orderItemRepo.find({
      where: {
        ...
        createdAt: MoreThanOrEqual(activeBday.startTime || activeBday.date),
        status: Not(OrderItemStatus.CANCELLED),
      },
      relations: ['createdBy', 'menuItem'],
    });
   ...
   */
  async getPeakIntensityPrediction(): Promise<any> {
    try {
      const now = new Date();
  ...
  async updateSoldQuantities(
    ...
    if (recentPrompts.length > 0) {
      ...
      if (userId) {
        ...
        const staff = await this.userRepo.findOne({ where: { id: userId } });
  ...
  async discoverComboRules(): Promise<any[]> {
    ...
    const excludeCategories = ['STORE', 'BILLIARD', 'INVENTORY', 'AKSESORIS'];
    ...
    for (const pair of Object.keys(pairSupport)) {
      ...
      const checkAndAddRule = (idA: string, idB: string) => {
  ...
  async getComboSuggestion(menuItemId: number): Promise<any> {
