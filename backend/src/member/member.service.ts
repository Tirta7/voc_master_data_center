...
export class MemberService {
    ...
    async topUp(id: number, amount: number, userId?: number, paymentMethod: string = 'CASH'): Promise<any> {
        if (this.toppingUp.has(id)) {
            throw new ConflictException('Proses top-up sedang berjalan untuk member ini.');
        }
        ...
        try {
            const member = await queryRunner.manager.findOne(Member, { where: { id }, relations: ['tier'] });
            if (!member) throw new NotFoundException('Member tidak ditemukan');

            // --- Validation ---
            const numAmount = Number(amount);
            if (!numAmount || numAmount <= 0) throw new BadRequestException('Nominal harus > 0');
            if (numAmount > 10_000_000) throw new BadRequestException('Maksimal Rp 10jt');
    ...
    async deductBalance(id: number, amount: number, manager?: any): Promise<Member> {
    ...
    async awardPoints(id: number, amount: number, manager?: any): Promise<Member> {
        const queryManager = manager || this.memberRepository.manager;
        const member = await queryManager.findOne(Member, { where: { id } });
        if (!member) throw new NotFoundException('Member not found');
     ...
     */
    async updateTotalSpend(id: number, amount: number, manager?: any): Promise<void> {
        const queryManager = manager || this.memberRepository.manager;
        try {
    ...
    async checkAndAutoUpgradeTier(member: Member, manager?: any): Promise<void> {
        ...
        try {
            ...
            const qualifyingTier = allTiers.find(
                (t: MemberTier) => t.autoUpgradeSpend !== null && currentSpend >= Number(t.autoUpgradeSpend)
            );
