...
export class WhatsAppService {
  ...
  constructor(private configService: ConfigService) {
    this.fonnteToken =
      this.configService.get<string>('FONNTE_TOKEN') || 'PLACEHOLDER_TOKEN';
  }
  ...
  async sendMessage(target: string, message: string) {
    try {
      ...
      this.logger.error(
        `Failed to send WA message to ${target}: ${error.message}`,
      );
      return null;
    }
  }
  ...
  async sendImage(target: string, message: string, url: string) {
    try {
      ...
      this.logger.error(
        `Failed to send WA image to ${target}: ${error.message}`,
      );
      return null;
    }
  }
