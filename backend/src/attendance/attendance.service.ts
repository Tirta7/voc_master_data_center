...
import { Repository, Between, Not, IsNull, LessThanOrEqual, MoreThanOrEqual, MoreThan } from 'typeorm';
...
import { Violation, ViolationType } from '../user/entities/violation.entity';
...
export class AttendanceService implements OnModuleInit {
  ...
  constructor(
    ...
    @InjectRepository(EmployeeShiftSchedule)
    private scheduleRepository: Repository<EmployeeShiftSchedule>,
    @InjectRepository(BusinessClosure)
    private closureRepository: Repository<BusinessClosure>,
    @InjectRepository(Violation)
    private violationRepository: Repository<Violation>,
    @InjectRepository(PayrollConfig)
    private payrollConfigRepository: Repository<PayrollConfig>,
  ...
  onModuleInit() {
    ...
    this.mqttService.subscribe('billiard/attendance/request_list', async (topic, payload) => {
      try {
        ...
        const logs = await this.attendanceRepository.createQueryBuilder('log')
          .leftJoinAndSelect('log.user', 'user')
          .where('log.date = :logicalDate', { logicalDate })
          .select([
        ...
        try {
          ...
          if (isFingerprint) {
            ...
            user = await this.userRepository.createQueryBuilder('user')
              ...
              .where('user.fingerprintData = :id', { id: fingerIdOnly })
              .orWhere('user.fingerprintData LIKE :start', { start: `${fingerIdOnly},%` })
              .orWhere('user.fingerprintData LIKE :mid', { mid: `%,${fingerIdOnly},%` })
              .orWhere('user.fingerprintData LIKE :end', { end: `%,${fingerIdOnly}` })
              .getOne();
          } else {
            // Coba cari dengan format asli ATAU format tanpa titik dua di kolom PIN atau RFID
            user = await this.userRepository.createQueryBuilder('user')
              .leftJoinAndSelect('user.role', 'role')
              .where('TRIM(UPPER(user.pin)) = :uid OR TRIM(UPPER(user.pin)) = :uidNoColons', { uid, uidNoColons })
              .orWhere('TRIM(UPPER(user.rfid)) = :uid OR TRIM(UPPER(user.rfid)) = :uidNoColons', { uid, uidNoColons })
              .getOne();
          }
          ...
          if (this.activeDualSession && (now - this.activeDualSession.timestamp) < 30000) {
            ...
            if (session.userId === user.id && session.type !== currentFactor) {
              ...
              await this.userRepository.query(
                `INSERT INTO violation (userId, type, description, penaltyAmount, createdAt, updatedAt) 
                 VALUES (?, 'SECURITY_VIOLATION', ?, 10000, NOW(), NOW())`,
                [session.userId, `Percobaan Dual Security Gagal: ${isFingerprint ? 'JARI' : 'KARTU'} TIDAK COCOK.`]
            ...
            this.activeDualSession = { userId: user.id, type: currentFactor, timestamp: now };
  ...
  async processCommand(userId: string, type: string, data?: any) {
    ...
    if (type === 'CAPTURE_FINGERPRINT' || type === 'START_FINGERPRINT_BATCH') {
      ...
      return { success: true, message: 'Biometric command sent to hardware' };
    }
    ...
    if (type === 'CLEAR_ALL_BIOMETRICS') {
      ...
      for (let i = 1; i <= 50; i++) {
        this.mqttService.publish(topic, {
          ...
          id: i,
          command: 'DELETE',
          name: 'SYSTEM WIPE', // Added name to avoid "UNKNOWN"
          msg: `ID ${i}`,
      ...
      return { success: true, message: 'Optimized sequential wipe broadcasted' };
    }
   ...
   */
  private async getAvailableShiftDefs(): Promise<ShiftDef[]> {
    const settings = await this.settingsService.getSettings();
    const raw: { name: string; startTime: string; endTime: string }[] =
      settings?.availableShifts || [];
    ...
    return raw.map((s) => {
      ...
      return {
        ...
        endTime: s.endTime,
        crossesMidnight: endMins < startMins, // e.g. 17:00-02:00 → 120 < 1020 → true
      };
    });
  }
   ...
   */
  async getShiftForEmployee(userId: number, logicalDate: string): Promise<ShiftDef | null> {
    const shiftDefs = await this.getAvailableShiftDefs();

    // 1. Check explicit schedule for this date
    const schedule = await this.scheduleRepository.findOne({
      where: { userId, date: logicalDate },
    });
  ...
  private determineStatus(
    ...
    return checkInMins > expectedMins
   ...
   */
  private async isBusinessClosed(logicalDate: string): Promise<boolean> {
    const count = await this.closureRepository.count({
      where: {
        startDate: LessThanOrEqual(logicalDate),
        endDate: MoreThanOrEqual(logicalDate),
      },
    });
    return count > 0;
  }
  ...
  private isRegistrationActive = false;

  async sendLcdPrompt(mode: string) {
  ...
  async checkIn(userId: number, note?: string): Promise<Attendance> {
    ...
    if (user.isVerified === false) {
      throw new BadRequestException('Akun Anda sedang dinonaktifkan (Unverified).');
    }
    const now = new Date();
    ...
    if (existing) {
      ...
      this.mqttService.publish('billiard/attendance/feedback', {
        name: user.name.substring(0, 16).toUpperCase(),
        status: (user.role?.name || 'STAFF') + ' | ' + now.toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' }),
        msg: 'SUCCESS'
      });
      return saved;
    }
  ...
  async checkInByPin(pin: string, note?: string): Promise<Attendance> {
    ...
    if (!user) throw new NotFoundException('PIN tidak terdaftar atau salah.');
  ...
  async checkOut(userId: number, note?: string): Promise<Attendance> {
    ...
    if (record.checkOutTime) {
      throw new ConflictException(`Sudah check-out hari ini pukul ${new Date(record.checkOutTime).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' })}`);
    }
    ...
    if (!isAdmin) {
      this.eventsGateway.attendanceUpdated({ type: 'ATTENDANCE_PENDING', data: saved });
    }
    return saved;
  }
  ...
  async approveAttendance(id: number, adminName: string): Promise<Attendance> {
    ...
    if (record.status === AttendanceStatus.LATE && shift && record.checkInTime) {
      try {
    ...
    this.eventsGateway.attendanceUpdated({ type: 'ATTENDANCE_APPROVED', data: saved });
    return saved;
  }
  ...
  async getTodayRecord(userId: number): Promise<Attendance | null> {
   ...
   async getHistory(
    ...
    from?: string,
    to?: string,
  ): Promise<(Attendance & { shiftName: string | null })[]> {
    ...
    if (records.length > 0) {
      ...
      const schedules = await this.scheduleRepository
        ...
        .where(
          '"userId" IN (:...userIds) AND "date" >= :from AND "date" <= :to',
          {
            userIds,
  ...
  async getSummary(
    ...
    const alpha = records.filter((r) => r.status === AttendanceStatus.ALPHA).length;
    const totalMinutes = records.reduce((sum, r) => sum + (r.workDurationMinutes || 0), 0);

    return { present, late, absent: 0, alpha, totalMinutes, records };
  }
  ...
  async createManual(
    ...
    if (existing) {
      ...
      this.eventsGateway.attendanceUpdated({ type: 'ATTENDANCE_MANUAL', data: savedExisting });
    ...
    const saved = await this.attendanceRepository.save(attendance);
    this.eventsGateway.attendanceUpdated({ type: 'ATTENDANCE_MANUAL', data: saved });
    return saved;
  }
  ...
  async deleteAttendance(id: number): Promise<void> {
    ...
    this.eventsGateway.attendanceUpdated({ type: 'ATTENDANCE_UPDATE', data: { id } });
  }
  ...
  /** Ambil jadwal shift karyawan untuk rentang tanggal */
  async getSchedules(from?: string, to?: string): Promise<EmployeeShiftSchedule[]> {
  ...
  /** Assign shift ke satu karyawan pada tanggal tertentu (upsert) */
  async assignShift(
    adminId: number,
    userId: number,
    date: string,
    shiftName: string,
    note?: string,
  ): Promise<EmployeeShiftSchedule> {
    // Validate shift name exists in settings
    const shiftDefs = await this.getAvailableShiftDefs();
    const valid = shiftDefs.find(
      (s) => s.name.toUpperCase() === shiftName.toUpperCase(),
    );
    if (!valid) {
      throw new BadRequestException(
        `Shift "${shiftName}" tidak ditemukan. Shift yang tersedia: ${shiftDefs.map((s) => s.name).join(', ')}`,
      );
    }
  ...
  /** Tukar shift antara dua karyawan pada tanggal tertentu */
  async swapShifts(
    adminId: number,
    userAId: number,
    userBId: number,
    date: string,
    reason?: string,
  ): Promise<{ scheduleA: EmployeeShiftSchedule; scheduleB: EmployeeShiftSchedule }> {
    if (userAId === userBId) {
      throw new BadRequestException('Tidak bisa menukar shift dengan diri sendiri.');
    }
    ...
    if (!shiftA) {
      throw new BadRequestException(
        `Karyawan A tidak memiliki jadwal shift pada tanggal ${date}. Assign shift terlebih dahulu.`,
      );
    }
    if (!shiftB) {
      throw new BadRequestException(
        `Karyawan B tidak memiliki jadwal shift pada tanggal ${date}. Assign shift terlebih dahulu.`,
      );
    }
  ...
  private async _upsertSchedule(
    data: Partial<EmployeeShiftSchedule>,
  ): Promise<EmployeeShiftSchedule> {
  ...
  async deleteSchedule(id: number): Promise<void> {
    const record = await this.scheduleRepository.findOne({ where: { id } });
    if (!record) throw new NotFoundException('Jadwal tidak ditemukan');
  ...
  async getClosures(): Promise<BusinessClosure[]> {
    return this.closureRepository.find({ order: { startDate: 'DESC' } });
  }

  async addClosure(startDate: string, endDate: string, reason: string): Promise<BusinessClosure> {
    const closure = this.closureRepository.create({ startDate, endDate, reason });
  ...
  async handleAlphaAttendance() {
    ...
    if (closed) {
      this.logger.log(
        `Business was CLOSED on ${logicalDate}. Skipping ALPHA detection.`,
      );
    ...
    this.logger.log(
      `Found ${allScheduled.size} employees scheduled for ${logicalDate}`,
    );
    ...
    for (const userId of allScheduled) {
      ...
      if (!existing) {
        ...
        this.logger.log(
          `Marking ${user?.name || `ID:${userId}`} as ALPHA for ${logicalDate}${shiftInfo}`,
        );
        ...
        try {
          ...
          this.logger.warn(
            `Failed to save ALPHA record for userId ${userId} on date ${logicalDate} due to unique constraint or race condition: ${error.message}`,
          );
        }
      }
    }
  ...
  private async _autoApprovePending(dateStr: string): Promise<void> {
    ...
    if (pendingRecords.length > 0) {
      this.logger.log(
        `Auto-approving ${pendingRecords.length} pending records for ${dateStr}`,
      );
   ...
   */
  private async _getScheduledEmployees(logicalDate: string): Promise<Set<number>> {
