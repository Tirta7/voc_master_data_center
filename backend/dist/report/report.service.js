...
let ReportService = class ReportService {
   ...
   */ async getAuditIntelligence(days = 7) {
        ...
        const cancelRankRaw = await this.auditRepository.createQueryBuilder('log').select('log.user', 'user').addSelect('COUNT(log.id)', 'count').where('log.createdAt >= :since', {
            since
        }).andWhere('log.action IN (:...actions)', {
            actions: [
                'CANCEL_REQUESTED',
        ...
        }).groupBy('log.user').orderBy('count', 'DESC').limit(10).getRawMany();
        // 2. Per-user suspicious (critical) actions total
        const suspiciousRankRaw = await this.auditRepository.createQueryBuilder('log').select('log.user', 'user').addSelect('COUNT(log.id)', 'count').where('log.createdAt >= :since', {
            since
        }).andWhere('log.action IN (:...actions)', {
            actions: SUSPICIOUS_ACTIONS
        }).groupBy('log.user').orderBy('count', 'DESC').limit(10).getRawMany();
        // 3. Hourly distribution of cancellations (detect abuse hours) — PostgreSQL syntax
        const hourlyRaw = await this.auditRepository.createQueryBuilder('log').select('EXTRACT(HOUR FROM log."createdAt")', 'hour').addSelect('COUNT(log.id)', 'count').where('log."createdAt" >= :since', {
            since
        }).andWhere('log.action IN (:...actions)', {
            actions: [
                'CANCEL_REQUESTED',
        ...
        }).groupBy('EXTRACT(HOUR FROM log."createdAt")').orderBy('count', 'DESC').getRawMany();
        // 4. Daily trend of critical actions (last N days) — PostgreSQL syntax
        const dailyRaw = await this.auditRepository.createQueryBuilder('log').select('CAST(log."createdAt" AS DATE)', 'date').addSelect('COUNT(log.id)', 'count').where('log."createdAt" >= :since', {
            since
        }).andWhere('log.action IN (:...actions)', {
            actions: SUSPICIOUS_ACTIONS
        }).groupBy('CAST(log."createdAt" AS DATE)').orderBy('date', 'ASC').getRawMany();
        // 5. Action breakdown breakdown by type (for pie chart)
        const actionBreakdownRaw = await this.auditRepository.createQueryBuilder('log').select('log.action', 'action').addSelect('COUNT(log.id)', 'count').where('log.createdAt >= :since', {
            since
        }).andWhere('log.action IN (:...actions)', {
            actions: SUSPICIOUS_ACTIONS
        }).groupBy('log.action').orderBy('count', 'DESC').getRawMany();
        ...
        const flaggedUsers = cancelRankRaw.filter((r)=>Number(r.count) >= anomalyThreshold).map((r)=>({
                user: r.user,
        ...
        return {
            ...
            cancelRanking: cancelRankRaw.map((r)=>({
                    user: r.user,
                    count: Number(r.count)
                })),
            suspiciousRanking: suspiciousRankRaw.map((r)=>({
                    user: r.user,
                    count: Number(r.count)
                })),
            hourlyPattern: hourlyRaw.map((r)=>({
                    hour: Number(r.hour),
                    count: Number(r.count)
                })),
            ...
            summary: {
                ...
                topCanceller: cancelRankRaw[0] ? {
                    user: cancelRankRaw[0].user,
                    count: Number(cancelRankRaw[0].count)
                } : null,
        ...
        return {
            ...
            summary: {
                ...
                staffPerformance: Object.entries(staffRevenue).map(([name, revenue])=>{
                    ...
                    const ratedTxs = transactions.filter((tx)=>(tx.createdBy?.name || 'System') === name || tx.openedBy?.name === name || tx.commissionUser?.name === name);
                    let sumRating = 0;
                    let countRating = 0;
                    ratedTxs.forEach((tx)=>{
    ...
    async getWeeklyTrafficTrend(days = 30) {
        const settings = await this.settingsService.getSettings();
        const [hours, minutes] = (settings.businessDayOffset || '00:00').split(':').map(Number);
        ...
        const aggregated = Array(7).fill(0).map((_, idx)=>({
                dayOfWeek: idx,
    ...
    async getDetailedStaffRatings(start, end) {
        let startDate;
        let endDate;
        if (start && end) {
        ...
        let topKasirMap = new Map();
        let topWaiterMap = new Map();
        for (const tx of transactions){
            ...
            // Aggregate Kasir
            if (!topKasirMap.has(kasirName)) topKasirMap.set(kasirName, {
                total: 0,
                count: 0
            });
            ...
            // Aggregate Waiter
            if (!topWaiterMap.has(waiterName)) topWaiterMap.set(waiterName, {
                total: 0,
                count: 0
            });
