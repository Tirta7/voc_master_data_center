...
async function run() {
  ...
  try {
    ...
    let output = '';
    tables.forEach(t => {
      output += `ID: ${t.id} | Name: ${t.tableName} | Pin: ${t.relayPin} | Light: ${t.isLightOn} | Status: ${t.status}\n`;
    });
    fs.writeFileSync('table_output.txt', output);
    console.log('Output written to table_output.txt');
  } catch (err) {
    console.error(err);
  } finally {
